	...
	describe('#foundation()', function() {
		it('should not wait for placeholders that are waiting for itself', function(done) {
			...
			let renderer = hawkejs.render('implement_with_foundation_extension', function doneImplement(err, result) {
				...
				try {
					assert.strictEqual(result.indexOf('window._initHawkejs = JSON.parse') > -1, true);
					assert.strictEqual(result.indexOf('window._hawkejs_static_expose = JSON.parse') > -1, true);
				} catch (err) {
					return done(err);
				}
