...
Template.setMethod(function interpret(variables) {
	...
	let pledge = Fn.series(function doExtensions(next) {
		...
		console.log('  »» Should do', that.extensions.length, 'extensions')
...
Template.setMethod(function waitForOtherTemplates() {
	...
	if (this.renderer.foundation_delays) {
		while (this.renderer.foundation_delays.length) {
			tasks.push(this.renderer.foundation_delays.shift());
		}
	}
	...
	for (i = 0; i < length; i++) {
		...
		// Don't wait for Renderer#addSubtemplate() promises that created this instance
		if (placeholder.rendering_templates && this.isDescendantOf(placeholder.rendering_templates)) {
			continue;
		}

		// Do not wait for manually created elements without a parent
		if (placeholder[Hawkejs.CREATED_MANUALLY] && (!placeholder.parentElement && !placeholder[Hawkejs.BLOCK])) {
			continue;
		}

		if (placeholder[Hawkejs.RENDER_PROMISE] && placeholder[Hawkejs.RENDER_PROMISE].state === 0) {
		...
		// 			placeholder._resolve_me_too = new Classes.Pledge.Timeout(30 * 1000, 'Render timeout, placeholder `_resolve_me_too` property was not called');
		// 		}
		// 	}
		// }

		// if (placeholder._resolve_me_too && placeholder._resolve_me_too.state === 0) {
	...
	if (tasks.length == 0) {
		...
		Fn.series(tasks).done(async function done(err) {
...
Template.setMethod(function render(variables) {
	...
	console.log('[[ Rendering Template', this, this.name, ']]');
...
Template.setMethod(function assemble() {
	...
	console.log('  »» Assembling Template Block ' + this.target_block_name);
