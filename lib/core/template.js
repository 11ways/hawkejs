...
Template.setMethod(Blast.checksumSymbol, function checksum() {
	return Blast.Bound.Object.checksum([this.name, this.theme, this.source_name, this.fnc]);
});
...
Template.setMethod(function interpret(variables) {
	...
	let pledge = Fn.series(function doImplementations(next) {
		...
		Fn.parallel(false, pre_tasks).done((err) => {
