...
Renderer.setMethod(function _renderQueue() {
	...
	Hawkejs.series(Fn.parallel(false, tasks), function doLayout(next) {
		...
		finish_pledge.done(function doneFinish(err, result) {
			...
			if (that.delayed_elements.length) {
				...
				Fn.parallel(false, tasks, function done(err) {
...
Renderer.setMethod(function doQueuedTasks() {
	...
	if (this.has_pre_finish_tasks) {
		...
		return Fn.parallel(false, tasks, err => {return err || true});
	}
});
