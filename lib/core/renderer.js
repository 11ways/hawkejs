...
Renderer.setMethod(function execExpressionFunction(fnc, variables, expression) {
	...
	fnc.call(this, this, __template, variables, helper, expression);
...
Renderer.setMethod(function doQueuedTasks() {
	...
	if (this.has_pre_finish_tasks) {
		...
		let tasks = Hawkejs.consume(this.pre_finish_tasks, (entry) => {
			return next => Hawkejs.doNext(entry(), next);
		});
		...
		return Fn.parallel(tasks, err => {return err || true});
	}
});
...
Renderer.setMethod(function printElement(name, options) {
	...
	if (options && options.body) {
		...
		if (!variables) {
			variables = this.active_variables || this.variables;
		}

		options.body.call(this, this, this.current_template, variables, null);
		this.closeElement(name);
	}
...
Renderer.setMethod(function applyElementOptions(element, options, for_sync_render) {
	...
	for (key in options.attributes) {
		...
		if (val != null) {
			...
			if (Hawkejs.needsToDoAsyncWork(val) && !for_sync_render) {
				...
				Hawkejs.addPreTask(element, function prepareValue() {
					...
					return Fn.series(final_tasks, function done(err) {
