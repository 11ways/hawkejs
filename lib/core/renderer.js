...
Renderer.setMethod(function createOpenElement(name, add_identifier) {
	...
	if (this.on_template_root_level) {
		let overlayed_element = this.addFallbackParentElementGetter(element);

		if (overlayed_element) {
			element = overlayed_element;
		}
	}
...
Renderer.setMethod(function addFallbackParentElementGetter(element) {
	...
	const $ancestor_element = this.current_variables.$ancestor_element;

	if (!$ancestor_element) {
		return;
	}

	// If it has, it'll be the fallback parentElement
	let result = Object.create(element, {
		parentElement: {
			...
			set: function setParentElement(new_parent) {
				return element.parentElement = new_parent;
			}
		}
	});
