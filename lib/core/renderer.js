...
Renderer.setCommand(function include(names, variables) {
	...
	if (names && typeof names == 'object' && names.expression) {
		names = this.parseExpression(names.expression);
	}
...
Renderer.setCommand(function setTitle(title) {
	...
	if (title && typeof title != 'string' && title.renderHawkejsContent) {
		const that = this;

		this.pre_finish_tasks.push(function getTitleContent() {
			return Hawkejs.callDeprecatedRenderContent(title, that);
		});
	}
...
Renderer.setCommand(function preload(href, options) {
	...
	if (this.hawkejs.css_preload_data) {
		let urls = this.hawkejs.css_preload_data.get(href_string);

		if (urls && urls.length) {
			let url;

			for (i = 0; i < urls.length; i++) {
				this.items_to_preload.push(urls[i]);
			}
		}
	}
