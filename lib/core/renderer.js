const TASK_GROUP = Symbol('task_group'),
      APPLIED_OPTIONS = Symbol('applied_options'),
...
Renderer.setMethod(function renderTemplate(templates, variables, main_name_modifier) {
	...
	let pledge = Hawkejs.series(this._emitBegin(templates), function doRender(next) {
...
Renderer.setMethod(function doTasksGrouped(tasks, callback) {
	...
	if (grouped_tasks) {
		Blast.Bound.Array.include(grouped_tasks, tasks);

		if (callback) {
			grouped_tasks.callbacks.push(callback);
		}
	...
	if (tasks.length > 3) {
		Fn.parallel(false, tasks).done(callback);
		return;
	}

	grouped_tasks = tasks;
	this[TASK_GROUP] = grouped_tasks;
	grouped_tasks.callbacks = [];
	...
	setTimeout(() => {
		...
		Fn.parallel(false, this.hawkejs.parallel_task_limit, grouped_tasks).done((err) => {

			let i;

			for (i = 0; i < grouped_tasks.callbacks.length; i++) {
				grouped_tasks.callbacks[i](err);
			}
		});
	});
});
...
Renderer.setMethod(function printElement(name, options) {
	...
	if (Hawkejs.Hawkejs.OPTIONAL_CLOSING_ELEMENTS[node_name]) {
		...
		while (parent_element) {
			if (parent_element.nodeName == node_name) {
...
Renderer.setMethod(function addSubtemplate(names, options, variables) {
	...
	// Make sure the variables are prepared (and properly cloned)
	if (this.active_variables) {
		this.active_variables = this.prepareVariables(this.active_variables);
	} else if (this.variables) {
		this.variables = this.prepareVariables(this.variables);
	}
...
Renderer.setCommand(function style(instructions, options) {
	...
	if (Blast.isBrowser && this.root_renderer === hawkejs.scene.general_renderer) {
...
Renderer.setCommand(function async(fnc, options) {
	...
	function getAsyncValue() {
		...
		if (!promise) {
			promise = new Blast.Classes.Pledge();
