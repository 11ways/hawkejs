const APPLIED_OPTIONS = Symbol('applied_options'),
...
/**
 * Promises the foundation element should wait for
 *
 ...
 * @type     {Array}
 */
Renderer.enforceRootProperty(function foundation_delays(value) {
	return value || [];
});
...
Renderer.enforceProperty(function language(value) {
	...
	if (this == this.root_renderer && this.html_element) {
		if (!value) {
...
Renderer.setProperty(function scope_id() {
	...
	if (this.root_renderer && this.root_renderer !== this) {
...
Renderer.setLocalProperty(function $0() {
	...
	let element = this.current_block.elements[this.current_block.elements.length - 1];
...
Renderer.setMethod(function execExpressionFunction(fnc, variables, expression) {
	...
	fnc.call(this, variables, expression);
...
Renderer.setMethod(function finishOnBrowser() {
	...
	if (this.dialogs) {
		tasks = tasks.concat(Hawkejs.prepareLineTasks(this.dialogs, this));
	}
...
Renderer.setMethod(function _emitBegin(templates) {
	...
	this.begin_event_pledges.push(pledge);
...
Renderer.setCommand(function getId(prefix) {
	...
	if (!prefix) {
		...
		result = prefix + '-' + this.prefix_id_numbers[prefix]++;
	}
...
Renderer.setCommand(function print(content, block) {
	...
	if (typeof content == 'string') {
		...
		let nodes = this.hawkejs.evaluate(content, null, true),
		    node,
		    i;
...
Renderer.setCommand(function printUnsafe(content) {
	...
	content.textContent = Bound.String.decodeHTML(content.textContent);
...
Renderer.setMethod(function ensureElementOptions(element) {
	...
	let need_sync_render = false,
	    ElClass          = element.constructor,
	    options          = element[OPTIONS];

	element[APPLIED_OPTIONS] = true;

	// Call the custom element constructor only when closing the element
	// We look for constitutors, because all custom elements have those
	if (ElClass.constitutors) {
...
Renderer.setMethod(function applyElementOptions(element, options) {
	...
	for (key in options.attributes) {
		...
		if (val != null) {
			...
			if (Hawkejs.needsToDoAsyncWork(val)) {

				let renderer = this,
				    value = val,
				    value_key = key;

				Hawkejs.addPreTask(element, function prepareValue() {
					...
					return Fn.series(final_tasks.concat(pre_tasks).concat(tasks), function done(err) {
				...
				if (key == 'data-he-name' && element.tagName != 'HE-BLOCK') {
	...
	if (options.directives) {
		...
		for (i = 0; i < options.directives.length; i++) {
			...
			let promise = context[method](element, val.value, options);

			if (promise && Blast.Classes.Pledge.hasPromiseInterface(promise)) {
				Hawkejs.addPreTask(element, promise);
			}
		}
	}
});
...
Renderer.setMethod(function fetch(href, options, callback) {
	...
	pledge.done(function done(err, result) {
		...
		if (err) {
			callback(err, err.result, request.xhr);
		} else {
...
function getFoundationContent() {
	...
	console.log('... Waiting for other templates!!!!!!!!!!!!!!!!')

	Fn.series(this.start_template.waitForOtherTemplates(), function waitForSiblings(next) {

		let length = renderer.placeholder_elements.length,
		    placeholder,
		    tasks = [],
		    i;

		console.log('-------------------------------------------')
		console.log('There are', length, 'placeholder_elements')

		for (i = 0; i < length; i++) {
			placeholder = renderer.placeholder_elements[i];

			if (placeholder.start_template == that.start_template) {
				if (placeholder === that) {
					continue;
				}

				console.log(' --------- ', placeholder, placeholder[Hawkejs.RENDER_PROMISE], placeholder[Hawkejs.RENDER_PROMISE].state)

				if (placeholder[Hawkejs.RENDER_PROMISE]) {
					tasks.push(placeholder[Hawkejs.RENDER_PROMISE]);
				}
			}
		}
		...
		if (tasks.length) {
			...
			tasks.push((next) => {
				that.start_template.waitForOtherTemplates().done(next);
			});
		...
		console.log(' ... Other templates ready!')
		_getFoundationContent.call(that, pledge, renderer, renderer.hawkejs, options);
	});

	// this.start_template.waitForOtherTemplates().then(() => {
...
Renderer.setCommand(function include(names, variables) {
	...
	if (names && typeof names == 'object' && names.expression) {

		let args = this.parseExpressionAsArguments(names.expression);
...
Renderer.setCommand(function createElement(name, add_identifier) {
	...
	if (element && element.constructor.constitutors) {
...
Renderer.setMethod(function createOpenElement(name, add_identifier) {
	...
	// For he-placeholders OR
	// if this custom element needs to render its contents,
	// treat it like a placeholder element (foundation needs to wait for this)
	if (name == 'he-placeholder' || (element.is_custom_hawkejs_element && element[Hawkejs.RENDER_CONTENT])) {
		
		

		// if (element.is_custom_hawkejs_element && element.tagName != 'HE-BLOCK') {
		// 	let msg = 'Render timeout: <' + element.tagName + '> `_resolve_me_too` property was not called';
		// 	element._resolve_me_too = new Classes.Pledge.Timeout(30 * 1000, msg);
		// 	element._resolve_me_too._he_custom_el = element
		// }
	}
...
Renderer.setCommand(function setTitle(title) {
	...
	if (title && typeof title != 'string' && Hawkejs.needsToDoAsyncWork(title)) {
		const that = this;
		...
		this.pre_finish_tasks.push(function getTitleContent() {
			...
			return Bound.Function.parallel(tasks);
		});
	}
...
Renderer.setCommand(function style(instructions, options) {
	...
	if (Blast.isNode && !options.inline) {
		let i;
