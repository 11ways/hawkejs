...
Renderer.setMethod(function printElement(name, options) {
	...
	if (name == 'he-placeholder' || (element.is_custom_hawkejs_element && element[Hawkejs.RENDER_CONTENT])) {
		...
		if (element.is_custom_hawkejs_element && element.tagName != 'HE-BLOCK') {
			element._resolve_me_too = new Classes.Pledge();
		}
	}
...
Renderer.setMethod(function registerElementInstance(element) {
	...
	var id = element.hawkejs_id || element.dataset.hid;
	...
	if (!id) {
		...
		if (this.root_renderer.variables.__he_elements.indexOf(element) > -1) {
			return;
		}
	}

	this.root_renderer.variables.__he_elements.push(element);
});
