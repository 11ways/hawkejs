...
Renderer.setStatic(function attachReactiveListeners(element, reactive) {
	...
	if (reactive.body?.values?.length) {
		reactive.body.values.forEach(optional => optional.addListener(() => {
	...
	if (reactive.properties) {
		attachReactiveElementUpdaters(element, setProperty, getProperty, reactive.properties);
	}

	if (reactive.states) {
		attachReactiveElementUpdaters(element, setState, getState, reactive.states);
	}
});
...
const getState = (element, key) => element.getState(key);
const setState = (element, key, val) => element.setState(key, val);
...
const attachReactiveElementUpdaters = (element, setter, getter, instructions) => {
	...
	for (let key in instructions) {
		...
		config.values.forEach(optional => optional.addListener(() => {
 ...
 * @param    {Function}                    setter
 * @param    {Function}                    getter
 * @param    {Develry.ObservableOptional}  optional
 * @param    {string}                      key
 * @param    {Object}                      config
 */
		...
		Classes.Pledge.Swift.done(content, (err, result) => {
			...
			if (typeof result == 'string') {
				if (should_be_escaped) {
					result = Hawkejs.Hawkejs.createTextNodeFromUnsafe(result);
				} else {
...
Renderer.setCommand(function printUnsafe(content) {
	...
	content = Hawkejs.Hawkejs.createTextNodeFromUnsafe(content);
...
Renderer.setMethod(function trackReactiveKeyExpression(data, type, key, expression) {
	...
	let reference_values = this.active_variables.getAllPaths(expression.$references).filter(entry => entry instanceof Classes.Develry.ObservableOptional);
...
Renderer.setMethod(function applyElementOptions(element, options, for_sync_render) {
	...
	if (options.states) {
		if (!element.is_custom_hawkejs_element) {
			throw new Error('Only custom elements can have states');
		}

		for (i = 0; i < options.states.length; i++) {
			let entry = options.states[i];
			let expression = entry.value;
			let key = entry.name;

			val = this.parseRuntimeExpression(expression);

			this.trackReactiveKeyExpression(reactive, 'states', key, expression);

			element.setState(key, val);
		}
	}
	...
	if (options.reference_count > 0 && options.references?.length) {
		...
		let values = this.active_variables.getAllPaths(options.references).filter(entry => entry instanceof Classes.Develry.ObservableOptional);
...
Renderer.setMethod(function registerElementInstance(element) {
	...
	if (!id) {
		...
		// Set the id
		Hawkejs.Element.Element.prototype.setIdentifier.call(element, id);
	} else {
