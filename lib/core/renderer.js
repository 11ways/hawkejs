...
Renderer.enforceProperty(function variables(value) {
	return value || new Variables(this);
});
...
Renderer.setMethod(function addSubtemplate(names, options, variables) {
	...
	let base_variables = Object.create(this.active_variables || this.variables);

	if (variables) {
		variables = Object.assign(base_variables, variables);
	} else {
...
Renderer.setCommand(function partial(names, variables, options) {
	...
	if (variables || t_vars) {
		variables = Object.assign(base_variables, variables, t_vars);
	} else {
		variables = base_variables;
	}
