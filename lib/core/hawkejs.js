 ...
 * @return   {String}
 */
Main.setMethod(function parseSyntax(source, name) {
	...
	for (i = 0; i < tokens.length; i++) {
		...
		if (token.type == 'name') {
			...
			if (!prevToken || (prevToken && prevToken.value != '.')) {
				...
				if (Bound.Array.flatten(fncVars).indexOf(token.value) > -1) {
					continue;
				}
...
Main.setMethod(function createScriptUrl(path, config) {
	...
	var url = Classes.RURL.parse(path, this.root_path + this.script_path);
...
Hawkejs.getTextContent = function getTextContent(line, renderer, level) {
	...
	if (typeof line === 'string') {
		...
		if (line.toHawkejsString) {
			result = line.toHawkejsString(renderer);
		} else if (line.outerHTML != null) {
...
Hawkejs.getElementContent = function getElementContent(line, renderer) {
	...
	if (typeof line === 'string') {
		...
		if (line[Blast.Classes.Hawkejs.RESULT] != null) {
			result = getElementContent(line[Blast.Classes.Hawkejs.RESULT]);
		}
	}
	...
	if (result && result.nodeName) {
		//Hawkejs.normalizeChildren(result, renderer);
	}
 ...
 * @param    {Array}     tasks
 */
Hawkejs.recurseLineTasks = function recurseLineTasks(lines, pre_tasks, tasks) {
