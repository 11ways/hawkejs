...
Main.setMethod(function dissectEjs(source) {
	...
	for (i = 0; i < lines.length; i++) {
		...
		if (line.type == 'inside') {
			...
			if (index > -1) {
				...
				adjustment = adjustment + (max_line_nr - line.lineStart);
				...
				for (j = i; j >= 0; j--) {
					...
					if (back_line.lineStart < max_line_nr) {
						break;
					}
	...
	for (i = 0; i < result.length; i++) {
		...
		line.adjusted_start = line.lineStart - line.adjustment;
	}
	...
	for (i = 0; i < lines.length; i++) {
		...
		if (line.type == 'inside') {
			...
			for (j = 0; j < pieces.length; j++) {
				...
				// inspecting later
				if (this.skip_set_err !== true) {
					piece_start = line_start + j;

					if (piece_start > line.lineMax) {
						piece_start = line.lineMax;
					}

					//block_code += TAB + '/*lineNr:' + piece_start + '*/ ';
				};
...
Main.setMethod(function handleError(renderer, templateName, lineNr, error) {
	...
	if (source && error && typeof error == 'object') {
		...
		} else if (error.stack.indexOf('Renderer.compiledView') > -1) {
			temp = Bound.String.after(error.stack, 'Renderer.compiledView').split('\n')[0].trim();
			temp = Bound.RegExp.execAll(/:(\d+):(\d+)\)/g, temp);
		...
		if (error.code) {
			...
			if (this.templates[templateName]) {
				...
				code = String(renderer.compiled_inlines[templateName]);
			}
		}
