...
Main.setMethod(function compileCodeToFunction(code, options) {
	...
	if (Blast.isNode) {
		...
		let context = Object.create(Blast.Globals);
		context.Hawkejs = Hawkejs;
		options.parsingContext = vm.createContext(context);
		...
		try {
			compiled = vm.compileFunction('return ' + code, [], options)();
		} catch (err) {
			error = err;
		}
	} else {
