...
Main.setStatic(function createElement(name) {
	return this.getInstance().createElement(name);
});
 ...
 * @return   {HTMLElement}
 */
Main.setMethod(function createElement(name) {
...
Main.setMethod(function handleError(renderer, templateName, lineNr, error) {
	...
	if (source && error && typeof error == 'object') {
		...
		if (at_eval) {
			...
			before_stack = Bound.String.before(error.stack, 'at Renderer.cpv_').split('\n');
		} else {
			before_stack = Bound.String.before(error.stack, 'at Renderer.compiledView').split('\n');
		}
 ...
 */
defStat(function prepareLineTasks(lines, renderer, force) {
	...
	Hawkejs.recurseLineTasks(lines, pre_tasks, tasks, renderer, force);
 ...
 * @return   void
 */
defStat(function recurseLineTasks(lines, pre_tasks, tasks, renderer, force) {
	...
	for (i = 0; i < length; i++) {
		...
		if (dirty_value === DIRTY_LINE_NEEDS_RENDER) {
		...
		if (children && children.length) {
			recurseLineTasks(children, pre_tasks, tasks, renderer, force);
		}
	}
});
...
defStat(function canBeMarkedAsDirty(line) {
	...
	return Hawkejs.needsToDoAsyncWork(line);
});
...
defStat(function markBranchAsDirty(line) {
	...
	while (line) {
		...
		line[DIRTY_INFO] = DIRTY_LINE_HAS_CHILDREN;
		line = line.parent;
	}
});
