...
Main.setMethod(function parseTemplateSyntax(source, name, wrap_ejs) {
	...
	for (i = 0; i < lines.length; i++) {
		...
		compiled = Hawkejs.Parser.Expressions.compileLine(config);
...
Main.setMethod(function stringifyToExpression(obj) {
	...
	if (this._debug) {
		...
		// or else the browser will thing it's an actual
		// script tag
		result = dried.replace(/\<\/script/g, "</s' + 'cript");
 ...
 * @return   {String}
 */
Hawkejs.informElementOfAssembly = function informElementOfAssembly(element, renderer) {
 ...
 * @return   {Array}
 */
Hawkejs.parseHTML = function parseHTML(html) {

	if (Blast.isNode) {
		throw new Error('Not yet implemented in Node');
	}

	let result = document.createRange().createContextualFragment(html);
 ...
 * @param    {Array}         new_children
 */
Hawkejs.appendChildren = function appendChildren(target, new_children) {
 ...
 */
Hawkejs.prepareLineTasks = function prepareLineTasks(lines, renderer) {
	...
	if (pre_tasks.length) {
		final_tasks.push(Fn.parallel(task_limit, pre_tasks));
	}
...
/**
 * Ensure custom elements have a RENDER_PROMISE property
 *
 * @author   Jelle De Loecker   <jelle@elevenways.be>
 * @since    2.1.3
 * @version  2.1.3
 ...
 * @param    {Object}    line
 */
function _ensureRenderPromise(line, renderer) {

	let nr = linenr++;

	let msg = 'Render timeout: <' + line.tagName + '> `RENDER_PROMISE` property was not resolved';

	let pledge = new Classes.Pledge.Timeout(30 * 1000, msg);
	pledge._he_custom_el = line;

	line[Hawkejs.RENDER_PROMISE] = pledge;

	console.log('      -- Waiting for', line.tagName)
	//renderer.foundation_delays.push(pledge);
	renderer.root_renderer.placeholder_elements.push(line);


	line[Hawkejs.RENDER_PROMISE].then(function() {
		console.log('      -- Finished line', nr, line, line.tagName)
	});
}
...
Hawkejs.recurseLineTasks = function recurseLineTasks(lines, pre_tasks, tasks, renderer) {
	...
	for (i = 0; i < lines.length; i++) {
		...
		if (line[Hawkejs.PRE_ASSEMBLE]) {
			...
			_ensureRenderPromise(line, renderer);
 ...
 * @return   {Pledge}
 */
Hawkejs.renderContent = function renderContent(line, renderer) {
	...
	if (!line[Hawkejs.RENDER_PROMISE]) {
		line[Hawkejs.RENDER_PROMISE] = new Classes.Pledge();
	}

	let result;

	console.log('      »» Rendering content of:', line, deprecated_method, line[Hawkejs.RENDER_PROMISE].state);

	if (deprecated_method) {
		result = Hawkejs.callDeprecatedRenderContent(line, renderer);
	} else {
		console.log('        -- RENDER_CONTENT is:', line[Hawkejs.RENDER_CONTENT])
		console.log('          -- OF:', line)
	...
	Hawkejs.doNext(result, () => {
		line[Hawkejs.RENDER_PROMISE].resolve();
	});
 ...
 * @return   {Pledge}
 */
Hawkejs.callDeprecatedRenderContent = function callDeprecatedRenderContent(line, renderer) {
 ...
 */
Hawkejs.addPreTask = function addPreTask(target, fnc) {
 ...
 * @return   {Boolean}
 */
Hawkejs.needsToDoAsyncWork = function needsToDoAsyncWork(obj) {
