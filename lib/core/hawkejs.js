...
defStat(function recurseLineTasks(lines, pre_tasks, tasks, renderer) {
	...
	for (i = 0; i < length; i++) {
		...
		let line_pre_tasks = line[Hawkejs.PRE_TASKS];

		// A line/element can have an array of tasks that should be performed first
		if (line_pre_tasks && line_pre_tasks.length) {
			while (line_pre_tasks.length) {
				let task = line_pre_tasks.shift();
		...
		if (children && children.length) {
			recurseLineTasks(children, pre_tasks, tasks, renderer);
		}
	}
});
