...
Main.setMethod(function _compile(options) {
...
Main.setMethod(function compileCodeToFunction(code, options) {
	...
	if (Blast.isNode) {
		...
		code = '(function vmCompiler(){return ' + code + '}())';
...
Main.setMethod(function handleError(renderer, templateName, lineNr, error) {
	...
	if (source && error && typeof error == 'object') {
		...
		} else if (error.stack.indexOf(templateName) > -1) {
			temp = Bound.String.after(error.stack, templateName).split('\n')[0].trim();
