...
Main.setMethod(function compile(options) {
	...
	try {
		template_code = this.interpretTemplate(options.template, name, safe_name);
	} catch (err) {
	...
	try {
		...
		console.log('Found error when compiling template ' + safe_name + ':');
		console.log(err);
		console.log(code);
			...
			if (temp !== false) {
				...
				if (temp.close || temp.name[0] == '/') {
					if (current) {
						...
						// If this is closing something (like /if)
						// but the parent is something that can't be closed
						// (like {% else %}), we need to get that parent's parent
						// @TODO: what about multiple branches that don't need closing?
						if (!current.options.close && current.parent && current.parent.options.type == type) {
...
Main.setMethod(function getFirstAvailableSource(names, callback) {
	...
	this.getFirstAvailableInternalSource(names, function gotResult(err, result) {
		...
		if (!result) {
			return callback(new Error('Failed to get templates "' + names + '", first available source is empty'));
		}

		if (!result.name && !result.source) {
			console.error('Illegal template source for', names, ':', result);
			return callback(new Error('Failed to get templates "' + names + '", no name & source was returned'));
		}
...
Hawkejs.getElementContent = function getElementContent(line, renderer) {
	...
	// If the line is a string, keep it as it is. It'll be added as a textnode
	if (typeof line === 'string') {
		result = line;
	} else if (line) {
 ...
 * @return   {String}
 */
function informSingleElementOfAssembly(element, renderer) {
	...
	if (children) {
		...
		for (i = 0; i < length; i++) {
			informSingleElementOfAssembly(children[i], renderer);
		}
	}
}
