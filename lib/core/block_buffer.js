...
BlockBuffer.setMethod(function _assemble(join_others) {
	...
	if (join_others && this.other_instances.length && this.options.content == 'push') {
		...
		return Fn.series(false, tasks, function joinedAll(err, result) {
	...
	return Fn.series(function doLineTasks(next) {
		...
		Fn.parallel(false, task_limit, line_tasks, next);
		...
		if (more_tasks.length) {
			Fn.parallel(false, task_limit, more_tasks, next);
		} else {
			next();
		}
	}, function done(err) {
