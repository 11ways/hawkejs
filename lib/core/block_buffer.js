...
BlockBuffer.setProperty(function is_assembled() {
	...
	if (this._assemble_pledge) {
		return !!this._assemble_pledge.is_done;
	}
...
BlockBuffer.setMethod(function push(line) {
	...
	if (Hawkejs.canBeMarkedAsDirty(line)) {
		mark_as_dirty = true;
	}
	...
	if (!length) {
		...
		result = this.lines.push(line);

		if (mark_as_dirty) {
			Hawkejs.markBranchAsDirty(line);
		}
	} else {
		...
		if (mark_as_dirty) {
			Hawkejs.markBranchAsDirty(line);
			Hawkejs.markBranchAsDirty(last);
		}
	}
...
BlockBuffer.setMethod(function toHTML() {
	...
	if (!this._assemble_pledge || !this._assemble_pledge.is_done) {
...
BlockBuffer.setMethod(function assemble() {
	...
	if (!this._assemble_pledge) {
		...
		this._assemble_pledge = new Classes.Pledge.Swift();
...
BlockBuffer.setMethod(function _assemble(join_others) {
	...
	let pledge = Hawkejs.series(function doLineTasks(next) {
		...
		// Do all required tasks of the lines + inform them they're being assembled (if required)
...
BlockBuffer.setMethod(function resolveBrowserTrims() {
	...
	for (key in this.trim_blanks) {
		...
		if (html.includes('he-placeholder')) {
			stop = true;
			break;
		}
