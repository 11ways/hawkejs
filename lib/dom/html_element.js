 ...
 * @param    {string[]} [node_names]      Only apply to these node names
 */
function definePropertyAttribute(property_name, attribute_name, node_names) {

	if (Array.isArray(attribute_name)) {
		node_names = attribute_name;
		attribute_name = null;
	}
	...
	if (node_names?.length) {
		...
		Element.setProperty(property_name, function getAttribute() {
			...
			if (node_names.includes(this.nodeName)) {
 ...
 * @param    {string[]} [node_names]      Only apply to these node names
 */
function defineBooleanPropertyAttribute(property_name, attribute_name, node_names) {
...
defineBooleanPropertyAttribute('disabled', ['BUTTON', 'FIELDSET', 'OPTGROUP', 'OPTION', 'SELECT', 'TEXTAREA', 'INPUT']);
defineBooleanPropertyAttribute('readonly', ['TEXTAREA', 'INPUT']);
defineBooleanPropertyAttribute('required', ['TEXTAREA', 'INPUT', 'SELECT']);
defineBooleanPropertyAttribute('draggable');

definePropertyAttribute('placeholder', ['INPUT', 'TEXTAREA']);
definePropertyAttribute('pattern', ['INPUT']);
