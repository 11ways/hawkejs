...
Element.setProperty(Hawkejs.PRE_ASSEMBLE, null);
...
Element.setProperty('_render_count', 0);
...
Element.setProperty(function outerHTML() {
	...
	if (VOID_ELEMENTS[this.nodeName] === 2) {
		html += '/>';
	} else {
		html += '>';
	}
...
Element.setMethod(function serializeProperties() {
	...
	if (this.attributes.length) {
		...
		if (this.attributes._items['for']) {
			result += this.getSerialized('for');
		}

		if (this.attributes._items['name']) {
...
Element.setMethod(function appendChild(child) {
	...
	if (typeof child == 'object') {
		...
		if (child.parentElement == this) {
			...
			index = child_nodes.indexOf(child);
