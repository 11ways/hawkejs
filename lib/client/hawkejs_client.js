...
Main.setMethod(function getFirstAvailableInternalSource(names, callback) {
	...
	if (!pledges[key]) {
		...
		// The `app_version` property can still be empty early in the boot
		// process (before `loadSettings` ran), fall back to the exposed
		// value: an unversioned url would get a stale cached response.
		let version = this.app_version || this.exposed('app_version');
