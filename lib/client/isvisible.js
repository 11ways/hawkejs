...
module.exports = function hawkejsElementVisible(Hawkejs, Blast) {
	...
	/**
	 * These overflow values make a "view context"
	 *
	...
	function getViewContext(element) {
		...
		if (!parent) {
			result = null;
		} else if (isDocumentElement(parent)) {
			result = parent;
		} else {
			...
			console.log(overflow, overflow_x, overflow_y);

			if (   makes_context[overflow]
				|| makes_context[overflow_x]
				|| makes_context[overflow_y])
			{
				result = parent;
			} else {
				result = getViewContext(parent);
			}
		}

		console.log('View context of', element, 'is', result);
