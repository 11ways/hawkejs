...
Scene.setMethod(function _updateBlock(renderer, block_name) {
	...
	if (renderer.assigns[block_name]) {
		element = renderer.assigns[block_name];
	} else {
		element = this.getBlockElement(block_name);
	}
...
Scene.setMethod(function getBlockElement(name, origin) {
	...
	var elements = document.querySelectorAll('[data-he-name="' + name + '"]');
...
Scene.setMethod(function setBodyOrigin(renderer) {
	...
	if (!template) {
		return console.warn('No last_template found');
	}
	...
	if (renderer.history !== false) {

		// @TODO: This currently sets the last template,
		// but it should be the entry template

		// Set the template origin
		//document.body.dataset.heTemplate = template.name;

		// Set the name
		//document.body.dataset.heName = block_name;

		// Set the active theme
		//document.body.dataset.heTheme = template.active_theme;
	}
...
Scene.setMethod(function _emitNewBlocks(renderer) {
	...
	for (i = 0; i < elements.length; i++) {
		...
		if (el.classList.contains('x-hawkejs')) {
			category = 'element';
		} else {
			category = 'block';
		}
		...
		this.loadScripts(this.scripts).done(function requiredScripts() {
...
Scene.setMethod(function _emitNewContent(renderer) {
	...
	for (let key in renderer.blocks) {
		...
		if (block.in_scope) {
			let parent = document.getElementById(block.in_scope);
...
Scene.setMethod(function loadScripts(scripts) {
	...
	scripts.forEach(function eachScriptGroup(script, key, index) {
		...
		console.log('Loading scripts', script.instructions)
...
Scene.setMethod(function generalPostRender(renderer) {
	...
	for (name in renderer.blocks) {
		...
		// @TODO: fix getting classes set by previous render!
...
Scene.setMethod(function moveBrowserFocus(element) {
	...
	if (element && element.expose_to_scene != null) {
		...
		for (i = 0; i < blocks.length; i++) {
			...
			element = this.getBlockElement(block.name);
	...
	if (element) {
		Classes.Element.prototype.forceFocus.call(element);
	}
});
