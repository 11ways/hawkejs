...
const Scene = Fn.inherits('Hawkejs.Base', function Scene(hawkejs) {
	...
	this.dialog_wrappers = document.getElementsByClassName('js-he-dialog-wrapper');
...
Scene.setMethod(function fetch(href, options, callback) {
	...
	options.headers['x-scene-id'] = this.getSceneId();
	options.url = url;
...
Scene.setMethod(function openUrl(href, options, callback) {
	...
	if (this.interceptOpenUrl) {
		...
		if (sub_pledge) {
			...
			sub_pledge.catch(function rejected(err) {
				...
				pledge.request = new_pledge.request;
				pledge.xhr = new_pledge.xhr;
				pledge.cancel = new_pledge.cancel;
	...
	if (Blast.DEBUG) {
		console.log('Opening URL', url, options, pledge);
	}
	...
	function handleResult(err, result) {
		...
		if (xhr.status == 202 && (location = request.getResponseHeader('location'))) {
			...
			setTimeout(function followLocation() {
				...
				pledge = that.fetch(location, {
					source_element: options.source_element,
					'x-hawkejs-request': true
				});

				// Make this the new "opening_url" pledge,
				// so that `isStillSameRequest()` function returns true
				that.opening_url = pledge;
...
Scene.setMethod(function handleServerResponse(url, options, payload, xhr) {
	...
	// Sometimes the url contains the hajax parameter, we should remove that
	if (history_url && history_url.indexOf('hajax=') > -1) {
		let temp_url = Blast.Classes.RURL.parse(history_url);
		temp_url.param('hajax', null);
		history_url = temp_url.href;
	}
...
Scene.setMethod(function onLinkClick(element, e) {
	...
	let active_request = element[ACTIVE_REQUEST];

	if (active_request) {
		let difference = Date.now() - active_request.request.time_started;
...
Scene.setMethod(function onFormSubmit(form, e) {
	...
	let active_request = form[ACTIVE_REQUEST];
	...
	let pledge = this.openUrl(action, options, function done(err) {
		...
		if (form.classList.contains('js-he-close-dialog')) {
			let dialog_wrapper = Hawkejs.closest(form, '.js-he-dialog-wrapper');
