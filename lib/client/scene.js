 ...
 */
Scene.setMethod(function _insertDialog(filter, el, variables, renderer) {
...
Scene.setMethod(function _updateBlock(renderer, block_name) {
	...
	console.log('Looking for', block_name, renderer.blocks[block_name], element)
...
Scene.setMethod(function getBlockElement(name, origin) {
	...
	element = this.findNamedElement(name);

	if (!element && block && block.renderer.dialog_open) {
		element = this.findNamedElement(block.renderer.dialog_open);
	}
