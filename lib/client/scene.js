const ACTIVE_REQUEST = Symbol('active_request'),
	  ...
	  SCROLL_POSITIONS = new Classes.Develry.Cache({limit: 50}),
      loaded_paths = {},
      loaded_names = {},
      loading_paths = {},
      loading_names = {},
...
const Scene = Fn.inherits('Hawkejs.Base', function Scene(hawkejs) {
	...
	// The current rendered page number
	this.page_number = 0;
...
Scene.enforceProperty(function allow_back_button(new_value, old_value) {
	...
	if (!new_value) {
		this.pushToHistory(null, document.URL);
...
Scene.setMethod(function openUrl(href, options, callback) {
	...
	// If the openUrl has a specific root element given,
	// it can run in parallel to a concurrent openUrl call
	let has_specific_root = !!options.root;
	...
	// Store the url on the pledge too
	pledge.url = url;

	if (!has_specific_root) {
	...
	if (options.blast_cache == null && options.cache == null) {
		options.blast_cache = false;
	}
	...
	function copyOverReq(other_pledge) {
	...
	function isStillSameRequest() {
		...
		if (has_specific_root || that.opening_url == pledge) {
			return true;
		}
	...
	function handleResult(err, result) {
		...
		let request = req_pledge.request,
		    popup = request?.getResponseHeader('x-hawkejs-popup'),
		    popup_url = request?.getResponseHeader('x-hawkejs-popup-url'),
		    hard_url = request?.getResponseHeader('x-hawkejs-navigate');
		...
		if (hard_url) {
			...
			if (popup) {
				...
				if (popup_url) {
					// An extra popup url means 2 links are being opened:
					// 1 redirect of the main website and 1 popup
					window.open(popup_url, 'hawkejs_popup', popup);
				} else {
					// No extra popup url means we only want the popup
					return window.open(hard_url, 'hawkejs_popup', popup);
				}
			}

			that.hard_navigating_to = hard_url;
			return window.location = hard_url;
		}
		...
		if (request && request.status == 202 && (location = request.getResponseHeader('location'))) {
 ...
 * @param    {Develry.Request} request    The request object
 *
 * @return   {Pledge}
 */
Scene.setMethod(function handleServerResponse(url, options, payload, request) {

	var history_url;

	// See if the server told us to use a different url for the history
	if (request) {
		history_url = request.getResponseHeader('x-history-url');
	...
	if (typeof payload == 'string') {
		...
		if (request) {
			let fallback = request.getResponseHeader('x-fallback-url');
	...
	try {
		exit_scroll_position = (document.scrollingElement || document.body).scrollTop;
	} catch (ignoreerr) {
		// Ignore
	}

	// Remember the scroll position
	SCROLL_POSITIONS.set(this.page_number, exit_scroll_position);

	// The (possible) new page number. Will be undefined if history is disabled
	let new_page_number;
	...
	if (options.history !== false && payload.history !== false) {

		// Increment the page number: the next state we're pushing is already for the new page
		this.page_number++;
		new_page_number = this.page_number;
		...
		state_content = Bound.JSON.toDryObject(state_content);
	...
	Hawkejs.series(false, function renderPayload(next) {
		...
		if (state_content && renderer.history !== false) {
			...
			that.pushToHistory(state_data, history_url);
		}
		...
		if (request && request.status >= 500) {
			if (payload.variables.message) {
				...
				err = new Error(request.status_message);
			}
...
/**
 * Push something to the history api
 *
 * @author   Jelle De Loecker   <jelle@elevenways.be>
 * @since    2.2.20
 * @version  2.3.6
 ...
 * @return   {Pledge}
 */
Scene.setMethod(function pushToHistory(data, url) {
	return this._modifyHistory('push', data, url);
});

/**
 * Replace the current entry in the history API
 *
 * @author   Jelle De Loecker   <jelle@elevenways.be>
 * @since    2.3.6
 * @version  2.3.6
 ...
 * @return   {Pledge}
 */
Scene.setMethod(function replaceHistory(data, url) {
	return this._modifyHistory('replace', data, url);
});

/**
 * Modify the history in some way
 *
 * @author   Jelle De Loecker   <jelle@elevenways.be>
 * @since    2.3.6
 * @version  2.3.6
 *
 * @param    {String}         method   Has to be one of: push, replace
 * @param    {Object}         data
 * @param    {String|Object}  url
 *
 * @return   {Pledge}
 */
Scene.setMethod(function _modifyHistory(method, data, url) {
	...
	try {
		if (typeof url != 'string' && Blast.Classes.RURL.isUrl(url)) {
			url = ''+url;
		...
		if (method === 'push') {
			...
			throw new Error('Unknown history API method: "'+method+'"');
		}
	} catch (err) {
		console.error('Failed to ' + method + ' history:', err);
	}
});
...
Scene.setMethod(function applyRenderResult(renderer, payload) {
	...
	let pledge = Hawkejs.parallel(false, [
...
Scene.setMethod(async function _updateContents(renderer, payload) {
	...
	for (block of renderer.blocks) {
		...
		if (this._updateBlock(renderer, block.name)) {
			updated = true;
		}
	}

	if (!updated && block_count && !renderer.root) {
		let template = renderer.last_template;

		if (template) {
			await template.assemble();

			let main_block = template.target_block;

			if (main_block) {
				await this.showAsDialog(main_block);
			}
		}
	}
...
/**
 * Show a block inside a dialog
 *
 * @author   Jelle De Loecker   <jelle@develry.be>
 * @since    2.2.13
 * @version  2.2.13
 *
 * @param    {Hawkejs.BlockBuffer}   block
 */
Scene.setMethod(async function showAsDialog(block) {
	...
	// A set `current_state` means this render pushed a history state:
	// the url is changing to the dialog's route (but hasn't yet at this
	// point), so remember the covered url for when the dialog closes
	let block_renderer = block.renderer,
	    root_renderer = block_renderer && (block_renderer.root_renderer || block_renderer);

	if (root_renderer && root_renderer.current_state != null) {
		dialog.previous_url = document.location.pathname + document.location.search + document.location.hash;
	}

	let div = document.createElement('div');
	div.setAttribute('slot', 'main');

	await block.assemble();

	Hawkejs.replaceChildren(div, block.toElements());
	dialog.append(div);
	...
	dialog.onHawkejsAssemble(block.renderer);
...
Scene.setMethod(async function _prepareDialog(renderer) {
	...
	let block = renderer.blocks.get(template.target_block_name);
	...
	// the url is changing to the dialog's route (but hasn't yet at this
	// point), so remember the covered url for when the dialog closes
	if (renderer.current_state != null) {
		dialog.previous_url = document.location.pathname + document.location.search + document.location.hash;
	}
 ...
 *
 * @return   {Boolean}   True if an existing element in the browser was updated
 */
Scene.setMethod(function _updateBlock(renderer, block_name) {

	let found_browser_element = false,
	    check_name = false,
	    element,
	    block = renderer.blocks.get(block_name);
	
	if (renderer.is_for_client_side !== false) {
		// It's probably a registered server-side renderer
...
Scene.setMethod(function getSceneId() {
	...
	if (!this.scene_id && this.exposed?.scene_id) {
		this.scene_id = this.exposed.scene_id;
	}
	...
	if (!this.scene_id) {
		...
		start_time = Obj.path(window, 'performance.timing.responseStart') || (Date.now() - ~~Blast.performanceNow());
...
Scene.setMethod(function registerServerRender(renderer, was_delayed) {
	...
	renderer.is_for_client_side = false;
	...
	if (typeof _hawkejs_static_expose != 'undefined') {
		...
		_hawkejs_static_expose = null;
	}

	// Store the exposed variables
	// Use toJSON() to properly extract values from the Variables instance
	if (renderer.expose_to_scene?.toJSON) {
		Object.assign(this.exposed, renderer.expose_to_scene.toJSON());
	} else {
	...
	history_state.dried = Bound.JSON.toDryObject(history_state.instance);
	...
	if (this.exposed.redirected_to) {
		// @TODO: clean this up
		//that.pushToHistory(null, this.exposed.redirected_to);
		that.replaceHistory(null, this.exposed.redirected_to);
	} else {
		//that.pushToHistory({content: history_state.dried}, url);
		that.replaceHistory({content: history_state.dried}, url);
	}

	Blast.emit('hawkejs_registered');

	Hawkejs.parallel(false, function loadScripts(next) {
		// Get scripts
...
Scene.setMethod(function attachHistoryListener() {
	...
	window.addEventListener('popstate', function onPopstate(e) {
		...
		// Unfortunately we don't know wether the user pressed back or forward,
		// so we can't deduce the wanted page number.
		let current_page_number = that.page_number;

		if (!that.allow_back_button) {
			console.log('Back button has been disabled, staying on', document.URL);
			that.pushToHistory(null, document.URL);
			return;
		}
		...
		if (e.state) {
			...
			Hawkejs.series(false, function preRenderEvent(next) {
				that.emit('preHistory', content, next);
			}, function renderPayload(next) {

				let payload = content.payload;

				if (payload && payload.is_for_client_side === false) {
					// Don't apply the server-side rendered blocks again,
					// they'll overwrite too much. Just re-render with the same variables.
					renderer = hawkejs.render(payload.queued_templates, payload.variables, next);
				} else {
					renderer = hawkejs.render(content.payload, next);
				}

				// Make sure this renderer doesn't also push their content to the history,
				// or else visitors will have to press the back button twice to get back
				renderer.history = false;

			}, function afterRender(next) {
				that.applyRenderResult(renderer, content.payload).done(next);
			}, function finished(err) {

				that.restoreHistoryScrollPosition(page_number);
			...
			// Go there, without pushing to the history API
			that.openUrl(returnLocation, {history: false});
		}
	});
 ...
 */
Scene.setMethod(function restoreHistoryScrollPosition(page_number) {

	if (page_number == null) {
		return;
	}

	let scroll_position = SCROLL_POSITIONS.get(page_number);

	// Also restore the page number
	...
	Blast.setImmediate(() => {
		(document.scrollingElement || document.body).scrollTop = scroll_position;
	});
});
...
Scene.setMethod(function _emitNewContent(renderer) {
	...
	return Hawkejs.series(this.loadScripts(this.scripts), function requiredScripts() {
		var entry,
		    i;
...
Scene.setMethod(function handleRendererScripts(renderer) {
	...
	if (renderer.scripts) {
		for (let parameters of renderer.scripts) {
			this._queueScript(...parameters);
		}
	}
...
Scene.setMethod(function _queueScript(instructions, options) {
	...
	if (!options.id) {
		options.id = Obj.checksum(options.instructions);
	}

	// First go over every script to see if it is unique
	this.scripts.every(function everyScript(item, key) {

		if (options.id == item.id || Obj.alike(options.instructions, item.instructions)) {
			if (options.reload) {
				item.reload = true;
			}
...
Scene.setMethod(function loadScripts(scripts) {
	...
	for (let script of scripts) {
		...
		if (script.series) {
			handler = Hawkejs.series;
		} else {
			handler = Hawkejs.parallel;
		}
...
Scene.setMethod(function setPageTitle(title) {
	...
	if (Hawkejs.needsToDoAsyncWork(title)) {
		let tasks = [];
		Hawkejs.recurseLineTasks([title], tasks, tasks, this.general_renderer);
		pledge = Hawkejs.parallel(tasks);
	}

	Hawkejs.doNext(pledge, () => {
...
Scene.setMethod(function _setPageTitle(title) {
	...
	let result = Hawkejs.getTextContent(title, this.general_renderer);

	if (title && typeof title == 'object' && result.indexOf('[object Object]') === 0) {
		return;
	}

	if (result) {
		result = Bound.String.stripTags(result);
		result = Bound.String.encodeHTML(result);
	}

	this.title_element.innerHTML = result;
});
...
Scene.setMethod(function getScript(options, force) {
	...
	if (typeof options == 'string') {
		...
		path = options;
	} else if (options instanceof Blast.Classes.Set) {
		path = Array.from(options);
	} else {
...
Scene.setMethod(function enableStyle(instructions, options) {
	...
	// Return the existing promise if the style is already being loaded
	if (loading_styles[path]) {
		return loading_styles[path];
	}

	let url = Classes.RURL.parse(path, this.hawkejs.root_path + this.hawkejs.style_path);
	...
	// Remove any version info again
	url.param('v', null);

	let plain_path = String(url);

	// Make sure the entry exists in the styles object
	if (!this.styles[plain_path]) {
		this.styles[plain_path] = {
			blocks: {},
			element: null
		};
	}
	...
	if (!data.element) {
		...
		let style = document.querySelector('link[rel="stylesheet"][href="' + path + '"]');

		if (!style && path != plain_path) {
			style = document.querySelector('link[rel="stylesheet"][href^="' + plain_path + '"]');
		}
		...
		if (!style) {
			...
			// Add onload callback
			style.onload = resolvePledge;

			style.onerror = function onStyleError(event) {
				loading_styles[original_path] = loading_styles[path] = null;
...
Scene.setMethod(function ajaxify() {
	...
	document.body.addEventListener('click', function onClick(event) {
		...
		// Ignore clicks that should open it in a new tab
...
Scene.setMethod(function onLinkClick(element, e) {
	...
	if (element.dataset.heScroll == 'false') {
		options.scroll = false;
	} else if (element.dataset.heScroll == 'true') {
		options.scroll = true;
	}
...
Scene.setMethod(function onFormSubmit(form, e, options) {
	...
	if (form.dataset.heScroll == 'false') {
		options.scroll = false;
	} else if (form.dataset.heScroll == 'true') {
		options.scroll = true;
	}
...
Scene.setMethod(function scrollTo(container, toElement, options) {
	...
	if (options.update_hash) {
		let id = toElement.id;

		// Unset the id to prevent a hard scroll
		toElement.id = '';
	...
	function animateScroll(now) {
		...
		if (now > end_time) {
			...
			if (options.focus) {
				that.moveBrowserFocus(toElement);
			}
...
Scene.setMethod(function appears(query, options, callback) {
	...
	// Add cleanup capability to the req function
