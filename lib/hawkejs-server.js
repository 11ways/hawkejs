	...
	hawkejs.enableClientSide = function enableClientSide (app, express, source, filepath) {
		...
		if (ejsStat) {
			...
			var clientfile      = fs.readFileSync(clientpath, 'utf-8'),
			    ...
			    domspotfile     = fs.readFileSync(path.resolve(__dirname, '..', 'vendor', 'domspot.js')),
			    foragefile      = fs.readFileSync(path.resolve(__dirname, '..', 'vendor', 'localforage.js')),
			    lzfile          = fs.readFileSync(path.resolve(__dirname, '..', 'vendor', 'lz-string.js')),
			...
			// Add domspot
			data += 'require.register("domspot.js", function(module, exports, require){\n';
			data += domspotfile + '\n' + 'module.exports = DOMSpot;';
			data += '});\n';
			...
			// Add the lz-string file
			data += 'require.register("lz-string.js", function(module, exports, require){\n';
			data += lzfile + '\n';
			data += '});\n';

			// Add the local forage file
			data += foragefile + '\n';
			...
			if (!this._debug) {
				...
				var result = uglify.minify(data, {fromString: true, mangle: false, compress: {unused: false}});
