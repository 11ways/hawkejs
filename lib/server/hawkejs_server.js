const URL_RX      = /[:,\s]\s*url\s*\(\s*(?:'(\S*?)'|"(\S*?)"|((?:\\\s|\\\)|\\\"|\\\'|\S)*?))\s*\)/gi,
...
/**
 * Extract preload data from CSS files
 * 
 ...
 * @param    {Object}    config
 */
Main.setMethod(async function extractCssPreloadData(config) {

	// CSS preload data
	if (!this.css_preload_data) {
		this.css_preload_data = new Map();
	}
	...
	} else if (file.source || file.code) {
		source = file.source || file.code;
	}
	...
	let matches = Bound.RegExp.execAll(URL_RX, source);

	if (!matches.length) {
		return;
	}

	let urls = this.css_preload_data.get(asset_path);

	if (!urls) {
		urls = [];
		this.css_preload_data.set(asset_path, urls);
	}
	...
	for (match of matches) {
		...
		url = Blast.Classes.RURL.parse(url, asset_path);

		// Make sure the hash is removed
		if (url.hash) {
			url.hash = '';
		}
		...
		if (lower.indexOf('.eot') > -1 || lower.indexOf('.woff') > -1 || lower.indexOf('.woff2') > -1 || lower.indexOf('.ttf') > -1) {
			as = 'font';
		} else {
			as = 'image';
		}
