...
(function() {
	...
	window.onload = function() {
		...
		// Ajaxify all the links
		hawkejs.ajaxifyLinks();

		// Emit en event saying this view is ready
		hawkejs.event.emit('viewready');
		...
		window.addEventListener('popstate', function(e) {
			...
			initialPop = !popped && location.href === initialURL && e.state === null;
			popped = true
			...
			if (e.state) {
				...
				if (e.state.zipped) {
					content = hawkejs.LZString.decompressFromUTF16(content);
				}

				// Parse the JSON content
				payload = JSON.parse(content);

				hawkejs.forceState($(document), payload);
				hawkejs.event.emit('historyChange', {element: $(document), payload: payload, event: e});
			} else {
	...
	hawkejs.ajaxifyLinks = function ajaxifyLinks() {
		...
		$('[data-hawkejs="form"]:not([data-hawkejs-ajaxified]) input[type="submit"],button[type="submit"]').click(function(e) {
			...
			if ($form.attr('data-file')) {
				...
				if ($form.attr('data-no-history')) {
					hawkejs.goToAjaxView(action, getData, postData);
				} else {
	...
	hawkejs.goToAjaxViewWithHistory = function goToAjaxViewWithHistory(url, getData, postData, callback) {
		...
		hawkejs.goToAjaxView(url, getData, postData, function(payload, jqXHR) {
			...
			if (payload.request && payload.request.elements) {
			...
			// If the object is too big and we're not on chrome, compress it
			if (!isChrome && text.length >= 320000) {
				stateData.zipped = true;
				stateData.content = hawkejs.LZString.compressToUTF16(text);
			} else {
				stateData.content = text;
			}

			// Register this state under the returned X-History-Url if it's set
			if (jqXHR.getResponseHeader('X-History-Url')) {
				url = jqXHR.getResponseHeader('X-History-Url');
			}
	...
	hawkejs.goToAjaxView = function goToAjaxView(href, getData, postData, callback) {
		...
		hawkejs.event.emit('ajaxbegin', href);

		// The function that will handle the response
		var handleResponse = function handleResponse(variables, textStatus, jqXHR) {
			...
			hawkejs.event.emit('ajaxdata', variables, function ajaxData() {
				...
				hawkejs.render(template, variables, $d, function($result, n, payload) {
					...
					if (callback) {
						callback(payload, jqXHR);
					}

					// Emit an event saying a render has finished on the client
					hawkejs.event.emit('renderend', payload);

					// Emit an event saying the view is now ready
					hawkejs.event.emit('viewready', payload);
				});
			});
		};
		...
		xhr.onreadystatechange = function(){
			...
			if (disposition && disposition.search('attachment') !== -1) {
				...
				if (typeof FileReader == 'undefined') {
					return jQuery.download(href, postData, 'post');
				}
				...
				if (typeof FileReader !== 'undefined') {
					...
					reader.onloadend = function() {
						...
						if (response.type == 'application/json-dry') {
							result = hawkejs.parse(result);
						} else if (response.type == 'application/json') {
							result = JSON.parse(result);
						}

						handleResponse(result, xhr.status, xhr);
					};
					...
					if (type.indexOf('json-dry') > -1) {
						result = hawkejs.parse(response);
		...
		if (postData) {
			...
			if (typeof href == 'string') {
				...
				// Get rid of IE's aggressive ajax cache
				href += cquery + 'hajax=' + Date.now() + '-' + (1 + Math.random() * 100);
			}
...
window.pr = function pr(message, extra) {
	...
	if (typeof message == 'object') {
		console.log('%c[DEBUG] %O', css, message);
	} else {
		console.log('%c[DEBUG] %c' + message, css, '');
	}
 ...
 * --------------------------------------------------------------------
 */
jQuery.download = function download(url, data, method) {
