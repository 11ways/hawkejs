...
function ajaxifyLinks () {
	...
	$('[data-hawkejs="form"]:not([data-hawkejs-ajaxified]) input[type="submit"]').click(function(e) {
		...
		goToAjaxViewWithHistory(action, getData, postData);
 ...
 */
var buildUrl = function buildUrl(existingURL, object) {
 ...
 */
function goToAjaxViewWithHistory (url, getData, postData, callback) {
	
	goToAjaxView(url, getData, postData, function(payload) {
		
		// Remove functions from the payload
		removeFunctions(payload);
 ...
 * @param    {Function}  callback    The function to callback to (with payload)
 */
function goToAjaxView(href, getData, postData, callback) {
	...
	if (getData) href = buildUrl(href, getData);
