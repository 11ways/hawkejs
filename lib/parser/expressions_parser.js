 ...
 */
var Eparser = Fn.inherits('Hawkejs.Parser.Token', function Expressions(source, builder) {
...
Eparser.setStatic(function compileBlock(config) {
	...
	let literal_count = 0,
	    all_tokens = Fn.tokenize(source, true),
	    nl_count,
	    lines = [],
	    token,
	    line = [],
...
Eparser.setStatic(function compileLine(line) {
	...
	// Tokenize the line
	let tokens = new Hawkejs.Parser.Expressions(line, config.builder);
		...
		switch (token.type) {
			...
			case 'punct':
				...
				if (token.name === 'dot') {
					...
					let path = call_expression[0].call;
