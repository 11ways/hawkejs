 ...
 *
 * @param    {Number}   level
 * @param    {Boolean}  in_literal   Are we in some kind of literal ({}, [])
 *
 * @return   {Object}
 */
Eparser.setMethod(function getExpression(level, in_literal) {
	...
	do {
		...
		if (token.value == '(') {
			...
			if (prev && !prev.keyword && entry) {
				...
				entry.arguments = this.getExpression(level + 1, in_literal);
				...
				// New level
				this.goToNext();
				result.push(this.getExpression(level + 1, in_literal));
			}
		...
		switch (token.type) {
			...
			case 'punct':
				if (in_literal && token.name == 'comma') {
					return result;
				}
				...
				if (token.name === 'dot') {
					...
					let call_expression = this.getExpression(level + 1, in_literal);
