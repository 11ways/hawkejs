...
Builder.setMethod(function closeFunction() {
	console.log('   Closing function', this.current)
...
Builder.setMethod(function closeStatement(name, args, options) {
	...
	console.log('Closing', name, options)
	this.closeFunction();
...
Builder.setMethod(function closeElement(entry) {
	...
	if (this.current.type != 'element') {
		...
		if (this.current.info) {
			type = this.current.info + ' ' + type;
		}

		throw new Error('Tried to close element ' + node_name + ', but found a ' + type + ' instead');
	}
	...
	let has_body = this.current.content.length > 0;
...
Builder.setMethod(function _compileRawExpression(entry) {
	...
	console.log('TOKENS:', tokens, entry.value)
...
Builder.setMethod(function _compileAttributeValue(val) {
	...
	if (val) {
		...
		if (val.type == 'string') {
			...
			let code = this.hawkejs.rewriteVariableReferences(val.value.slice(2, -2));
			result = R(code);
