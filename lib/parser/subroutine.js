 ...
 */
const Subroutine = Fn.inherits(null, 'Hawkejs.Parser', function Subroutine(builder, body) {

	// The builder instance
	this.builder = builder;

	// The root hawkejs instance
	this.hawkejs = builder.hawkejs;

	// The original body instance
...
Subroutine.setMethod(function toString() {
	...
	if (this.builder.is_inline) {
		if (this.original_body?.reference_count > 0) {
			result += this.name + '.source_code = ' + JSON.stringify(this.original_body.getSourceCode()) + ';\n';
		}
	} else {
		result += this.name + '.source_name = ' + JSON.stringify(this.builder.template_name) + ';\n';
	}
