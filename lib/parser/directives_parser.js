...
Dparser.setMethod(function convert() {
	...
	for (i = 0; i < grouped.length; i++) {
		...
		if (entry.type == 'open_tag') {
			...
			if (has_attributes || entry.directives || entry.properties || has_codes) {
				result += ', {';
				...
				if (has_codes) {
					if (entry.directives || entry.properties || has_attributes) {
						result += ', ';
					}
					...
					for (j = 0; j < entry.codes.length; j++) {
						...
						if (token.type == 'expressions') {
							code = Hawkejs.Hawkejs.prototype.parseSyntax(code);
						}
