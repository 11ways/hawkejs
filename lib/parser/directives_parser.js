	...
	blocks: [
		{
			...
			can_replace: [Blast.HTML_TOKENIZER_STATES.TAG_NAME],
		},
		{
			name  : 'code',
			open  : '<%',
...
const Dparser = Fn.inherits('Hawkejs.Parser.Token', function Directives(source, options) {
	...
	// Also keep the `html_only` option in mind
	if (this.allow_code == null && options.html_only != null) {
		this.allow_code = !options.html_only;
	}
...
Dparser.setMethod(function build(options) {
	...
	if (!builder) {
		...
		builder.is_inline = (options?.is_inline ?? this.options?.is_inline) || false;
	}
	...
	for (i = 0; i < grouped.length; i++) {
		...
		if (in_comment && entry.type != 'comment') {
			// Ignore things inside comments
		} else if (entry.type == 'comment') {
			// Ignore comments

			if (in_comment || entry.value[0] == '<') {
			...
			builder.add({type: 'source', source: entry.source});
...
Dparser.setMethod(function parseCurrent() {
	...
	if (current.type == 'open_bracket' && next) {
		if (next.type == 'tag_name' || next.type == 'expressions') {
			result.type = 'open_tag';

			if (next.type == 'tag_name') {
				result.tag_name = this.next().value;
			} else {
				result.tag_name_expression = this.next().value;
			}
		
			this.parseAttributes(result);

			if (!this.current) {
				throw new Error('Unexpected open tag encountered');
			}
			...
			if (next.type == 'expressions') {
				result.tag_name_expression = next.value;
			} else {
				result.tag_name = next.value;
			}
...
/**
 * Fix some token source code
 * (In case the tokenizer stripped some delimiter)
 *
 * @author   Jelle De Loecker   <jelle@elevenways.be>
 * @since    2.4.0
 * @version  2.4.0
 ...
 * @return   {string}
 */
Dparser.setMethod(function getTokenSource(token) {
