 ...
 * @return   {String}
 */
Dparser.setMethod(function convert(options) {
...
console.log(grouped)
	...
	for (i = 0; i < grouped.length; i++) {
		...
		if (entry.type == 'open_tag') {
			...
			console.log('Compile config:', config)

			let compiled = Hawkejs.Parser.Expressions.compileBlock(config);

			console.log('Compiled:', compiled, 'from', entry);
...
Dparser.setMethod(function parseCurrent() {
	...
	if (current.type == 'open_bracket' && next) {
		...
		if (!result.newlines_skipped) {
			result.newlines_skipped = 0;
		}
