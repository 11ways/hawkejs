...
function renderCustomTemplate(re_render) {
	...
	var template = this.constructor.compiled_template || this.constructor.template_file;
	...
	return Fn.series(function getRenderVariables(next) {
		...
		that.hawkejs_renderer.renderTemplate(template, render_vars, String(c++)).done(next);
		...
		// Are we specifically re-rendering an existing element?
		// Then we need to already make sure we get valid elements.
		if (re_render) {
			lines = block.toElements();
		}

		// This works fine on the server, because the lines will be normalized later
		// but on the client, this will push placeholders into the DOM
...
Element.setMethod(function connectedCallback() {
	...
	var has_introduced = typeof this.introduced == 'function',
	    has_template = !!(this.constructor.template_file || this.constructor.compiled_template),
	    that = this,
	    data;
