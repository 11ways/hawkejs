...
function renderCustomTemplate(re_render) {
	...
	let pledge = Hawkejs.series(function getRenderVariables(next) {
		...
		if (result) {
			if (Classes.Pledge.isThenable(result)) {
				Classes.Pledge.done(result, (err, result) => {

					if (err) {
						return next(err);
					}

					render_vars = Classes.JSON.clone(result, 'toHawkejs');
					next();
				});
		...
		if (that[Hawkejs.VARIABLES]) {
			Object.assign(render_vars, that[Hawkejs.VARIABLES]);
		}
...
Element.setMethod(function onHawkejsCloseElement() {
	...
	if (this.has_rendered) {
		...
		let slot_data = _extractSlotData.call(this, false);
...
Element.setMethod(function getVariable(name) {
	...
	if (variables && variables[name] != null) {
 ...
 * @return   {Object}
 */
function _extractSlotData(clear_content) {
