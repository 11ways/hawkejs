...
function renderCustomTemplate(re_render) {
	...
	Hawkejs.series(next => {
		...
		Classes.Pledge.Swift.done(that.prepareRenderVariables(), (err, result) => {

			if (err) {
				return next(err);
			}

			render_vars = Hawkejs.Variables.cast(result, renderer);
			render_vars = Bound.JSON.clone(render_vars, 'toHawkejs');
		...
		render_vars.setFromTemplate('self', that);
...
Element.setMethod(function rerender() {
	...
	if (client_rects?.width) {
		...
		this.style.width = ~~(client_rects.width) + 'px';
		this.style.height = ~~(client_rects.height) + 'px';
	}
