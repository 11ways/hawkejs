...
const CURRENT_RENDER = Symbol('current_render'),
      ...
      STATE_VALUES = Hawkejs.STATE_VALUES,
...
const getAllStateValues = (element) => {
	...
	const renderer = element.hawkejs_renderer;

	if (renderer) {
		// The element might have not initiated fully,
		// make sure that happens now
		renderer.ensureElementOptions(element);
	}
...
Hawkejs.getAllStateValues = getAllStateValues;
...
function renderContentsWithTemplate(element, template, variables, slot_data, pledge) {
	...
	// Make sure custom element state values are initialized
	if (element.is_custom_hawkejs_element) {
		getAllStateValues(element);
	}
...
Element.setMethod(function setState(name, value) {
	...
	// @TODO: What if the value *is* an optional too?
...
Element.setMethod(function connectedCallback() {
	...
	let event_handlers = this[Hawkejs.EVENT_HANDLERS];

	if (event_handlers) {
		Element.ensureEventHandlers(this, event_handlers, true);
	}
...
/**
 * Check if an element is inside a nested custom element
 * (i.e., has a custom element ancestor that is not the root element)
 *
 * @author   Jelle De Loecker <jelle@elevenways.be>
 * @since    2.4.2
 * @version  2.4.2
 *
 * @param    {HTMLElement}   root_element    The element we're extracting slots from
 * @param    {HTMLElement}   child           The element to check
 *
 * @return   {boolean}
 */
function isInsideNestedCustomElement(root_element, child) {

	let parent = child.parentElement;

	while (parent && parent !== root_element) {
		if (parent.is_custom_hawkejs_element) {
			return true;
		}
...
function _extractSlotData(clear_content, re_render) {
	...
	if (re_render) {
		...
		for (i = 0; i < slot_elements.length; i++) {
			...
			// Skip elements that are inside nested custom elements
			// (they belong to those elements, not to us)
			if (isInsideNestedCustomElement(this, child)) {
				continue;
			}
...
function _insertSlotData(slot_data) {
	...
	let processed = new Set(),
	    slots = this.querySelectorAll('slot'),
	    slot,
	    name,
	    i;
	...
	// Fallback to old slot logic for elements that don't use the <slot> element
	for (name in slot_data.slots) {
		if (processed.has(name)) {
			continue;
		}

		if (!slots) {
			slots = Hawkejs.getElementsByAttribute(this, 'data-he-slot')
		}
...
Element.setMethod(function rerender() {
	...
	// Possible (reactive) render instructions
	const instructions = this[Hawkejs.RENDER_INSTRUCTION];

	if (IS_CUSTOM_ELEMENT && !this.inner_template && !instructions) {
		return;
	}
	...
	// Do the reactive rendering first
	// This renders the actual original body
	if (instructions) {
		try {
			this[CURRENT_RENDER] = null;
	...
	if (IS_CUSTOM_ELEMENT) {
		...
		// If the original body source is rendering,
		// we have to wait for that!
		if (pledge) {
			pledge = Classes.Pledge.Swift.waterfall(
				pledge,
				() => renderCustomTemplate.call(this, true),
			);
		} else {
