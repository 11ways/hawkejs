...
function _resolveRender(err) {
	...
	console.log('Resolving render of', !!this._resolve_me_too)

	if (this[Hawkejs.RENDER_PROMISE]) {
		this[Hawkejs.RENDER_PROMISE].resolve(!err);
	}
...
Element.setMethod(function applyCompiledTemplate(fnc) {
	...
	Blast.defineGet(vars, 'self', () => {

		// Ensure the element options are ready
		renderer.ensureElementOptions(this);

		return this;
	});

	let elements = renderer.evaluate(fnc, vars),
	    i;
