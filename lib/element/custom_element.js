...
function ElementConstructor() {
	...
	if (this.constructor._has_default_attributes === true) {
		let name,
		    conf;

		for (name in this.constructor._attributes) {
			conf = this.constructor._attributes[name];
	...
	// If the custom element has a simple, HTML-only template set,
	// render the contents immediately
	if (this.constructor.compiled_template && this.constructor.compiled_template.plain_html != null) {

		// If there are already child elements, do not render the template
		if (this.childElementCount) {
			return;
		}

		// If there are child *nodes*, make sure they're not empty
...
if (Blast.isNode) {
	...
	if (typeof customElements != 'undefined') {
		...
		document.createElement = function createElement(name) {
			...
			var element = Hawkejs._createUnconstructedElement.apply(null, arguments);
...
Hawkejs._createUnconstructedElement = function _createUnconstructedElement(name) {
	...
	} else if (Blast.isBrowser || !Hawkejs.Hawkejs.elements[name]) {
...
function renderCustomTemplate(re_render) {
	...
	if (this.constructor.use_new_renderer_scope) {
		renderer = this.hawkejs_renderer.createSubRenderer();
 ...
 * @param    {Boolean}  plain_html    Is this plain HTML?
 */
Element.setStatic(function setTemplate(template, plain_html) {
...
Element.setMethod(function toDry() {
	...
	if (Blast.isNode && this.hawkejs_renderer && !this.hawkejs_id) {
		this.hawkejs_renderer.registerElementInstance(this);
	}

	if (Blast.isNode && this.hawkejs_renderer && this.hawkejs_renderer.is_for_client_side) {
		serialize_all = true;
	} else if (Blast.isNode && !this.hawkejs_renderer) {
		serialize_all = true;
	}
...
Element.setMethod(function afterRender(callback) {
	...
	this.addEventListener('rendered', function afterRender() {
		callback.call(that);
	});
});
...
Element.setMethod(function _renderTemplateSynchronously() {
	...
	if (Blast.isNode) {
		this._setInnerHTML(this.constructor.compiled_template);
	} else {
 ...
 * @param    {Boolean}            delay_foundation   [true]
 */
Element.setMethod(function delayAssemble(task, delay_foundation) {

	if (typeof task == 'function') {
		task = task.call(this);
	}

	if (delay_foundation == null && Blast.isNode) {
		delay_foundation = true;
	}

	if (delay_foundation && this.hawkejs_renderer && this.hawkejs_renderer.foundation_delays) {
		this.hawkejs_renderer.foundation_delays.push(task);
	}
