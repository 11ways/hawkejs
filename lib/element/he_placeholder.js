 ...
 * @version  2.0.0
 */
const Placeholder = Fn.inherits('Hawkejs.Element', 'HePlaceholder');
...
Placeholder.setMethod(function introduced() {
	...
	if (id && this.parentElement && Hawkejs.browser_store[id]) {
		...
		this.remove();
	} else if (!this.attempted_manual_render) {
		this.attempted_manual_render = true;
		this.getContent(() => {
			this.introduced();
		});
	}
...
Placeholder.setMethod(Hawkejs.RENDER_CONTENT, function renderPlaceholderContent(renderer) {
	...
	if (tasks.length) {
		final_tasks.push(function doTasks(next) {
			Hawkejs.parallel(false, tasks).done(next);
		});
	}

	return Hawkejs.parallel(false, final_tasks, function done(err, result) {
