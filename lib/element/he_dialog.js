 ...
 * @version  2.0.1
 */
const Dialog = Fn.inherits('Hawkejs.Element', 'HeDialog');
...
Dialog.setMethod(function close() {
	...
	// If this dialog was opened by a navigation (the scene stamped the
	// covered url on it), restore that url. Inline dialogs
	// (`showDialog()`) never changed the url, so they don't get the
	// property and nothing is restored.
	if (this.previous_url != null && typeof history != 'undefined') {
		try {
			history.pushState(null, null, this.previous_url);
		} catch (err) {
...
Dialog.setMethod(function getBlockUpdateTarget() {
	...
	if (this.dataset.heSlot) {
		...
		target = this.querySelector('.he-dialog-contents');
	}
