var supports_prevent_scroll,
	...
	interval_ids = Symbol('interval_ids'),
    ProtoEl,
    Element,
    m_proto;
...
Blast.definePrototype(Element, 'moveAfterElement', function moveAfterElement(reference_node) {
	...
	if (!reference_node.parentElement) {
		return false;
	}

	if (reference_node.parentElement != this.parentElement) {
		reference_node.parentElement.append(this);
	}
...
Blast.definePrototype(Element, 'getScreenreadableElement', function getScreenreadableElement() {
	...
	// Prefer full heading elements
	if (name && name[0] == 'H' && isFinite(name[1])) {
		return this;
	}
...
/**
 * Add an interval callback that will run as long as the element is part
 * of the dom
 *
 * @author   Jelle De Loecker   <jelle@elevenways.be>
 * @since    2.2.17
 ...
 * @return   {Number}
 */
Blast.definePrototype(Element, 'addIntervalListener', function addIntervalListener(fnc, ms) {

	if (!ms || ms < 1) {
		throw new Error('Unable to set an interval listener without a valid interval time');
	}
...
/**
 * Hide the element for assistive technology.
 *
 * @author   Jelle De Loecker   <jelle@elevenways.be>
 * @since    2.3.7
 * @version  2.3.7
 ...
 * @param    {Boolean}   value
 */
Blast.definePrototype(Element, 'hideForAssistiveTechnology', function hideForAssistiveTechnology(value = true) {

	this.removeAttribute('data-original-aria-hidden');
 ...
 * This will also make sure the `aria-hidden` attribute is set to true,
 * because some automatic checkers will complain if it's not.
 *
 * @author   Jelle De Loecker   <jelle@elevenways.be>
 * @since    2.3.7
 * @version  2.3.7
 ...
 * @param    {Boolean}   value
 */
Blast.definePrototype(Element, 'hideForEveryone', function hideForEveryone(value = true) {

	let original_aria_hidden_value = this.dataset.originalAriaHidden;

	if (value) {

		if (!original_aria_hidden_value) {
			this.dataset.originalAriaHidden = this.ariaHidden;
		}
		...
		if (original_aria_hidden_value) {
			this.ariaHidden = original_aria_hidden_value;
			this.removeAttribute('data-original-aria-hidden');
		}
