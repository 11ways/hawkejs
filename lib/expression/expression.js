...
Expression.setMethod(function getTokenValue(token, vars) {
	...
	if (token.object) {
		return this.getTokenLiteralValue(token.object, vars);
	}
 ...
 * @param    {Object}  vars      An object of variables
 */
Expression.setMethod(function getTokenLiteralValue(entries, vars) {
	...
	for (i = 0; i < entries.length; i++) {
		entry = entries[i];
		result[entry.key] = this.parseExpression(entry.value);
	}
...
Expression.setMethod(function callPathWithArgs(path, args, vars) {
	...
	if (path.length == 1) {
		...
		if (typeof class_path[0] == 'object') {
			temp = this.parseExpression(class_path[0], vars);
