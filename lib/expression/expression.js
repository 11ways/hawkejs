const R = Hawkejs.Parser.Parser.rawString,
      ...
      ObservableOptional = Classes.Develry.ObservableOptional,
      Variables = Hawkejs.Variables;
 ...
 * @version  2.0.0
 */
const Expression = Fn.inherits(null, 'Hawkejs.Expression', function Expression(view) {
...
Expression.setMethod(function overlayElementVariables(variables, element) {
	...
	variables = Variables.cast(variables, this.view);
...
Expression.setMethod(function _getValueByPath(path, variables, main_variables) {
	...
	for (i = 0; i < path.length; i++) {
		...
		if (current instanceof Variables) {
...
Expression.setMethod(function parseExpression(tokens, vars, call_chain) {
	...
	for (i = 0; i < tokens.length; i++) {
		...
		let keyword = toLowerCase(token.keyword);
		...
		if (!have_a_value) {
			...
			operator = token;
			op = toLowerCase(operator.keyword);
		} else {
...
const toLowerCase = (input) => input ? input.toLowerCase() : input;
...
Expression.setMethod(function getTokenValue(token, vars) {
	...
	if (typeof token.value != 'undefined') {
		result = token.value;
	} else if (typeof token.domain == 'string') {
		result = vars.getDomainValue(token);
	...
	// Unwrap optionals
	if (result && this.unwrap_optionals && result instanceof Optional) {
		result = result.value;
	}
...
Expression.setMethod(function callPathWithArgs(path, args, vars) {
	...
	let options = [this.view.helpers, this.view, Blast.Globals],
	    context,
	    result,
	    temp,
	    fnc,
...
/**
 * Apply a "where" expression on keys and values
 ...
 * @return   {*}
 */
Expression.setMethod(function applyWhere(where, keys, values, as, as_key) {
	...
	return {keys: new_keys, values: new_values, length: new_keys.length};
});