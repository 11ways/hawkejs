...
With.setMethod(function execute() {
	...
	if (options.where && length) {
		...
		for (i = 0; i < length; i++) {
			...
			vars = new Hawkejs.Variables(this.view);
			vars.setFromTemplate(options.as, value);
	...
	let main_vars = this.vars.overlay();

	// Add the "$amount" to it
	main_vars.setFromTemplate('$amount', length);

	// Set some extra variables
	main_vars.setFromTemplate('_$each_as', options.as);
	main_vars.setFromTemplate('_$each_var', this.variable);
	main_vars.setFromTemplate('_$each_values', this.values);
	main_vars.setFromTemplate('_$each_keys', this.keys);
...
With.setMethod(function onBranch_single(options, fnc) {
	...
	vars.setFromTemplate(options.as, this.values[0]);
