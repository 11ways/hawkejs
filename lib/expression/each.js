...
Each.setStatic(function parseArguments(options) {
	...
	if (tokens.hasValue(',')) {
		let as_key_var = tokens.getVariable(1);
		if (as_key_var && as_key_var.path) {
			result.as_key = as_key_var.path[0];
		}
		tokens.next();
	}
...
Each.setMethod(function execute() {
	...
	} else if (this.vars.has('_$each_var')) {
		variable = this.vars.get('_$each_var');
		as_name = this.vars.get('_$each_as');
		values = this.vars.get('_$each_values');
		keys = this.vars.get('_$each_keys');
	} else if (this.parent && this.parent.constructor.name == 'With') {
		base_vars = this.parent.prepareVars();
		variable = base_vars.get('_$each_var');
		values = base_vars.get('_$each_values');
		as_name = base_vars.get('_$each_as');
		keys = base_vars.get('_$each_keys');
	}
	...
	if (variable && keys.length) {
		...
		for (i = 0; i < keys.length; i++) {
			...
			vars = base_vars.overlay();
			vars.setFromTemplate('$index', i);
			vars.setFromTemplate('$key', keys[i]);

			if (as_key) {
				vars.setFromTemplate(as_key, keys[i]);
			}

			vars.setFromTemplate('$value', values[i]);
			vars.setFromTemplate(as_name, values[i]);
