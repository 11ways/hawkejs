...
hp.render = function render (template, variables) {
	...
	if (!ClientSide && variables.hawkejs) this._extractReqData(variables);

	// See if the client can render this page himself
	if (!ClientSide && variables.hawkejs.ajax && this._clientSideRender) {
		console.log('Client is requesting a page he can render himself');
		var ajaxpayload = this._preparePayload(variables, true);
		ajaxpayload.hawkejs.renderPart = template;
		
		// Send the ajaxpayload to the client
		return ajaxpayload;
	}

	// Create a new, clean payload
	// This will be available to all the elements being rendered
	var payload = this._preparePayload(variables);
	
	// See if the template actually exists
	...
	do {
		...
		doResult = this._EjsRender(doTemplate, payload);
	...
	this._joinBlocksBuffer(payload);
...
hp.renderUpdate = function renderUpdate (template, variables, $object) {
	...
	if (payload.request.expand.length) {
		...
		var expandinto = payload.request.expand.shift();
		
		// Convert the dom name
		var domname = expandinto.replace(/\//g, '__');
		
		var $selection = µ.select($object, '[data-hawkejs-from-template="' + domname + '"]');
		
		console.log('Looking for existing temlate ' + domname);
		console.log('-- Length ');
		...
		if (!$selection.length) {
			this.renderUpdate(expandinto, payload, $selection);
		} else {
			startUpdate = true;
		}
	}
 ...
 */
hp._EjsRender = function _EjsRender (templatename, payload, fromstring) {
	
	// Get the template
	var templateSource = this.getTemplate(templatename);
