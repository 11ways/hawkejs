 ...
 */
hawkejs.utils.inject = function inject(target, first, second) {
...
hawkejs.renderFile = function renderFile (path, options, callback) {
	...
	this.render(elementname, options, false, function ($result, sendObject, payload) {
		...
		returnObject = utils.getHtml($result, true);
		...
		var afterImplementCall = function() {
			...
			if (payload.request.expand.length) {
				...
				for (i = 0; i < expansionArray.length; i++) {
					...
					if ($updateDoc) {
						$selection = utils.select($updateDoc, 'hawkejs[data-hawkejs-from-template="' + templateName + '"]');
			...
			if (!payload.request._renderTemplatesHasCalledBack && !IO && !wait) {
				...
				if (!$updateDoc || !$updateDoc.length) $updateDoc = utils.objectify(rHtml);
...
hawkejs._preparePayload = function _preparePayload (variables, template, forAjax, callback) {
	...
	// Variables to put in the hawkejs client side storage
	payload.__storageVars = {};
	...
	try {
		...
		temp = clone(variables);
	} catch (err) {
		console.log('ERROR cloning variables:' + err);
	}
	...
	try {
		// Clone the helpers
		payload['__helpers'] = clone(passHelpers);
	} catch (err) {
		console.log('ERROR cloning helpers: ' + err);
	}
	...
	middle.start(function hawkejsMiddlewareDone() {
		callback(payload);
	});
};
...
function clone(obj, sel) {
	var result = (obj ? _clone('', obj, sel) : obj);
	return result;
}

/**
 * Deep clone an object: internal function
 *
 * @author   Jelle De Loecker   <jelle@develry.be>
 * @since    0.1.2
 * @version  0.1.2
 */
function _clone(pth, src, sel, seen) {
	...
	id = seen.index.indexOf(src);

	if (id > -1) {
		return seen.values[id];
	}

	seen.index.push(src);
	seen.values.push(ret);
		...
		if (val && typeof val =='object') {
			if     (val instanceof Boolean) { val=Boolean(val);        }
				...
				if (!Array.isArray(val) && val.constructor && val.constructor.name != 'Object') {
					// Don't clone object instances, it'll mess them up
				} else {
					val = _clone(sub, val, sel, seen);
				}
			}
		}
...
/**
 * Force states based on payload, used for HTML5 History API
 *
 * @author   Jelle De Loecker   <jelle@codedor.be>
 * @since    0.0.13
 * @version  0.1.2
 ...
 */
hawkejs.forceState = function forceState($document, payload) {
	this.doInsertions($document, payload);
	this._sendEvents($document, payload);
	this.applyChanges($document, payload);
};
...
hawkejs._addParentClass = function _addParentClass($document) {
	...
	var elements = utils.select($document, '[data-add-parent-class]'),
	    element,
	    value,
	    i;
...
hawkejs._sendEvents = function _sendEvents ($document, payload) {
	...
	var render_variables = merge({}, payload.scope.variables),
	...
	// @todo: emit an event for destroyed implementations
	// (these still need to be added to the destroyed object, first)

	// Emit an event for created implementations
	for (key in used.implementations) {
		
		entry = used.implementations[key];
...
hawkejs._matchLocation = function _matchLocation ($origin, payload) {
	...
	for (elementid in temp) {
		...
		$this = utils.select($origin, '#' + elementid);
...
hawkejs._applyAttributes = function _applyAttributes ($this, ins, previous) {
	...
	if (ins && ins.status) {
		...
		for (var elementid in ins.status) {
			...
			// Select the target
			var $target = utils.select($this, '#' + elementid);
...
hawkejs._insertImplementation = function _insertImplementation ($origin, payload) {
	...
	var $spaces = utils.select($origin, 'hawkejs[data-hawkejs-implement]');
	...
	for (var i = 0; i < spaceNr; i++) {
		...
		var $this = utils.objectify($spaces[i], $origin);
		...
		if (payload.request.elements[elementname]) {
			...
			var $clone = utils.clone(b); //utils.objectify(b.html);
			//var $clone = utils.objectify(b.html);
			//log($clone, 1);
			//var t = $clone.clone();
			//
			...
			$this.html(hawkejs.unescape$(utils.getHtml($clone)));
		} else {
...
hawkejs._insertBlock = function _insertBlock ($origin, payload, blockchain) {
	...
	var $spaces = utils.select($origin, 'hawkejs[data-hawkejs-space]');
	...
	for (var i = 0; i < spaceNr; i++) {
		...
		if (payload.request.blocks[blockname]) {
			...
			if (ClientSide) {
				...
				var $destroyed = utils.select($this, 'hawkejs[data-hawkejs-block]');
				
				// Go over every block we're about the destroy
				for (var dd = 0; dd < $destroyed.length; dd++) {
					
					// Create a jQuery object
					var $d = utils.objectify($destroyed[dd], $destroyed);
			...
			var injectHtml = '<hawkejs id="hawkejs-insert-block-' + blockname + '" '
			               ...
			               + templatedomname + '">'
			               + hawkejs.unescape$(utils.getHtml($clone))
			               + '</hawkejs>';
...
hawkejs._extendClientVar = function _extendClientVar (name, value, $element, payload) {
	...
	if (ClientSide) {
		...
		// Do a deep extend
		merge(this.storage[name], clone(value));
	} else {
		// This code is sent to the client, he will then do the code above us
...
hawkejs.escape$ = function escape$(html) {
	...
	html = html.replace(/\<html(?!\w)/g, '<hawkejshtml data-hawkejs-shim="html"');
	html = html.replace(/\<\/html(?!\w)/g, '</hawkejshtml');
	html = html.replace(/\<head(?!\w)/g, '<hawkejshead data-hawkejs-shim="head"');
	html = html.replace(/\<\/head(?!\w)/g, '</hawkejshead');
	html = html.replace(/\<body(?!\w)/g, '<hawkejsbody data-hawkejs-shim="body"');
	html = html.replace(/\<\/body(?!\w)/g, '</hawkejsbody');
 ...
 * @returns  {string}           The escaped html
 */
hawkejs.unescape$ = function unescape$(html) {
...
hawkejs.addHtml = function addHtml(html, $element, payload, options) {
	...
	$target = utils.select($element, '[data-block="' + options.block + '"]');
	...
	if (!$target || !$target.length) {
		...
		$target = utils.select($element, options.block);
	} else {
		type = 'before';
	}
...
hawkejs._addHtml = function _addHtml ($element, destination, html) {
	...
	if (doselection) {
		...
		if (addType === 'prepend') {
			$dest = utils.select($element, '[data-destination="' + datadest + '"]');
			after = true;
		}

		if (!$dest || !$dest.length) {
			after = false;
			$dest = utils.select($element, destination);
		}
	}
...
hawkejs._addHtmlNew = function _addHtml ($element, destination, html) {
	...
	$im = utils.select($element, destination);
	...
	$dest = utils.select($im, '[data-destination="' + name + '"]', $super);
...
hawkejs._EjsRender = function _EjsRender (templatename, payload, options, callback, block) {
	...
	var dialEjs = function dialEjs (templateSource) {
		...
		merge(ejsPayload, boundHelpers);
		merge(ejsPayload, prive);
...
hawkejs.storeTemplate = function storeTemplate (name, source) {
	...
	var buffercode = '<% render_start(buf, __stack) %>',
	    endcode = '<% render_end() %>';
 ...
 */
hawkejs.utils.cloneobject = function(superObj, extension) {
...
hawkejs.order = function order(obj, options) {
	...
	ordered.sort(function(a, b) {
		...
		var aVal = Number(hawkejs.getObjectPath(a, options.mainName)),
		    bVal = Number(hawkejs.getObjectPath(b, options.mainName)),
		    aSub,
		    bSub;
		...
		if (aVal > bVal) {
			...
			if (options.subName) {
				...
				aSub = Number(hawkejs.getObjectPath(a, options.subName)),
				bSub = Number(hawkejs.getObjectPath(b, options.subName));
...
/**
 * Get a value from an object at the given path
 *
 * @author   Jelle De Loecker   <jelle@codedor.be>
 * @since    0.0.12
 * @version  0.0.12
 ...
 * @return   {Array}               An array containing the ordered items
 */
hawkejs.getObjectPath = function getObjectPath(obj, path) {
 ...
 */
hawkejs.utils.cloneSafe = function cloneSafe(original) {
	return JSON.parse(JSON.stringify(original));
};

hawkejs.clone = hawkejs.utils.cloneSafe;
...
if (ClientSide) {
	hawkejs.spot = require('domspot');
	hawkejs.LZString = require('lz-string');
}
 ...
 *
 * @param    {Object}   obj      The original object
 * @param    {Array}    results  The array containing the value objects
 * @param    {String}   basepath The current path
 * 
 * @return   {Array}
 */
hawkejs.formify = function formify(obj, results, basepath) {
	...
	// If the base object is an array, set a basepath name
	if (isArray && !basepath) {
		return formify(obj, results, 'unnamed');
	}
	...
	for (key in obj) {
		...
		// If the current item is not an object, set it as the value
		if (typeof obj[key] != 'object') {
			results.push({name: path, value: obj[key]});
		} else {
			// If it is, recurse
			formify(obj[key], results, path)
		}
	}
...
hawkejs.arrayBufferToString = function arrayBufferToString(buf) {
	return String.fromCharCode.apply(null, new Uint16Array(buf));
};

hawkejs.stringToArrayBuffer = function stringToArrayBuffer(str) {
