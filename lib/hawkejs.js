...
// Multiply a string
// @author http://blog.stevenlevithan.com/archives/fast-string-multiply
...
// Debug helper
var log = function log (messages, recursive) {
	
	if (!recursive) recursive = 1;
	
	if (recursive == 1) console.log ('\n>>>>>>>>>>>>>>>>>>>>>>>>>>')
	
	if (typeof messages == "string") {
		console.log(mul8('   ', recursive) + messages);
	} else {
		/*for (var i in messages) {
			console.log(mul8('   ', recursive) + i + ':');
			log(messages[i], (recursive + 1));
		}*/
...
hp.renderFile = function renderFile (path, options, callback) {
	...
	console.log('Rendering file ' + path + ' took ' + t.get() + 'ms');
}
...
hp.render = function render (template, variables, $update, callback) {
	...
	if (variables.hawkejs) {
		...
		if (variables.hawkejs.ajax && this._clientSideRender) {
			...
			console.log('Client is requesting a page he can render himself');

			// Clear certain elements from the payload and send to the client
			payload = this._preparePayload(variables, true);
			payload.hawkejs.renderPart = template;
			
			console.log('Preparing ' + template + ' took ' + t.get() + 'ms');
			
			// Send the payload to the client instead of HTML
			return payload;
		}
	}
	// <Noclient
	
	// If update is true, the given variables already contain payload information
	// This object will be used throughout the render
	payload = this._preparePayload(variables, update);
	
	// Add the wanted template to the payload if it's an update
	if (update) payload.hawkejs.renderPart = template;
	...
	this._renderTemplates(template, payload, $update, function($result) {
		...
		if (payload.request.expand.length) {
			...
			for (var i in payload.request.expand) {
				...
				$result = thisHawk._postEjsRender(expansionElement, payload);
			}
			
		} else {
	
			// No expansions were requested, so just finalize the original element
			$result = thisHawk._postEjsRender($result, payload);
		...
		finalHtml = µ.getHtml($result, true);
 ...
 * @param    {function} callback   The callback to make. If none, sync
 */
hp._renderTemplates = function _renderTemplates (template, payload, $update, callback, block) {
	...
	this._EjsRender(template, payload, false, function (resultObject) {
		...
		var afterImplementCall = function() {
			...
			if (payload.request.expand.length) {
				...
				// Can we stop looking?
				// If it's not an update, we can never stop looking
				var stopLooking = false;
				
				// If it is an update, see if we can find our wanted destination
				if ($update) {
					var $selection = µ.select($update, '[data-hawkejs-from-template="' + templateName + '"]');
					stopLooking = $selection.length;
				}
			
				// Keep on looking if it wasn't found
				if (!stopLooking) {
					thisHawk._renderTemplates(expansionTemplate, payload, $update, callback, block);
				} else {
					startUpdate = true;
				}
			...
			if (startUpdate) {
				console.log('We can start updating after finding template ' + template + '!');
			} else {
				console.log('Did not find template ' + template);
			}
			...
			if (!payload.request._renderTemplatesHasCalledBack) {
				...
				// Create the document with ejs' result html
				var $result = µ.objectify(rHtml);
...
/**
 * Render an update to an existing document
 * 
 * @author   Jelle De Loecker   <jelle@kipdola.be>
 * @since    2013.01.22
 * @version  2013.01.22
 ...
 * @param    {object}   variables   The variables (or payload)
 * @param    {object}   $object     The target document
 */
hp.renderUpdate = function renderUpdate (template, variables, $object) {
	
	// If this is the client side, we probably still need to add helpers
	if (ClientSide) {
		var payload = this._preparePayload(variables);
	} else {
		var payload = variables;
	}
	
	// Add this template to the update block chain
	payload.request.templateChain.unshift(template);
	
	// Render the template with EJS
	doResult = this._EjsRender(template, payload);
	
	// Can we start updating the page with the blocks we have?
	var startUpdate = true;
	...
	for (var i = 0; i < payload.request.implement.length; i++){
		...
		this._EjsRender(implementTemplate, payload);
	...
	if (payload.request.expand.length) {
		...
		var $selection = µ.select($object, '[data-hawkejs-from-template="' + templateName + '"]');
	
		// Keep on looking if it wasn't found
		if (!$selection.length) {
			this.renderUpdate(expansionTemplate, payload, $object);
		} else {
			startUpdate = true;
		}
	}
 ...
 * @param    {boolean}  forAjax     Is this payload meant to be sent back?
 *
 ...
 * @version  2013.01.20
 *
 * @param    {object}   $result        The object to postrender
 * @param    {object}   payload        The payload
 */
hp._postEjsRender = function _postEjsRender ($result, payload) {

	// Insert implementations
	// This function is also called inside _insertBlock()!
	this._insertImplementation($result, payload);
	
	// Insert blocks into spaces
	this._insertBlock($result, payload);
...
hp._EjsRender = function _EjsRender (templatename, payload, fromstring, callback, block) {
	...
	if (payload.request.elements[templatename]) {
		console.log('Tried to start EJS render for already rendered template: ' + templatename);
	...
	var dialEjs = function dialEjs (templateSource) {
		...
		// Render using ejs
		var renderHtml = ejs.render(templateSource, ejsPayload);
	...
	// Get the template
	var templateSource = this.getTemplate(templatename, getTemplateCallback);
 ...
 */
hp.getTemplate = function getTemplate (name, callback) {
