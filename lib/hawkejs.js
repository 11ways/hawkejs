	...
	this._EjsRender(template, payload, false, function (resultHtml) {
		...
		var afterImplementCall = function() {
			...
			if (payload.request.expand.length) {
				...
				log(' == Expanding template ' + expansionTemplate);
 ...
 */
hp._preparePayload = function _preparePayload (variables, template, forAjax) {
 ...
 */
hp._EjsRender = function _EjsRender (templatename, payload, options, callback, block) {
	...
	if (this._requireCache[name]) {
		...
		if (this._baseDir) {
			...
			if (!callback) {
				...
				var at = new Timer();
				log('\nGetting "' + name + '" asynchronously ...', false, 'verbose');
				...
				if (!ClientSide) {
					fs.readFile(getPath, 'utf-8', function (err, result) {
						...
						log('Got template "' + name + '" asynchronously in ' + at.get() + 'ms\n', false, 'verbose');
					});
				} else {
					// Client side
