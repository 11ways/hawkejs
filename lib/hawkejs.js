		...
		var afterImplementCall = function() {
			...
			if (startUpdate) {
				if (foundTemplateSpace) {
					console.log('We can start updating after finding template "' + templateName + '" to put ' + template + ' in!');
				} else {
					if (!payload.request._renderTemplatesHasCalledBack) {
						console.log('We probably need a complete new document for template "' + template + '"');
					} else {
						console.log('Callback has already fired, home found for template "' + template + '"');
					}
				}
			} else {
				console.log('Did not find place to put template "' + template + '"');
			}
