module.exports = function hawkejsSceneClass(Hawkejs, Blast) {
	...
	var Scene = Blast.Classes.Informer.extend(function Scene(parent) {
		...
		console.log('Forms:', this.newForms)
	...
	Scene.setMethod(function serialize_form(_form) {
		...
		for (i = 0; i < form.elements.length; i++) {
			...
			// Skip certain fields
			if (!field.name || field.disabled || ~skip_types.indexOf(field.type)) {
				continue;
			}
			...
			if (field.type == 'select-multiple') {
				...
				for (j = form.elements[i].options.length - 1; j >= 0; j--) {
					if (field.options[j].selected) {
						value.push(field.options[j].value);
					}
				}
			...
			Blast.Bound.Object.setPath(data, field.name.replace(/\]$/, '').split(/\[|\]\[|\]/g), value);
		}
	...
	Scene.setMethod(function ajaxify() {
		...
		while (this.newForms.length) {
			...
			form.addEventListener('submit', function onSubmit(e) {
				...
				var form_data = that.serialize_form(this),
				    options = {},
				...
				that.openUrl(this.action || location.href, options);
			});
	...
	Scene.setMethod(function updateData(id, data) {
		...
		arr.forEach(function eachEntry(entry, index) {
			...
			// Split the path
			pieces = element.getAttribute('data-path').split('.');
			...
			// Revive the function
			if (!entry.fnc) {
				entry.fnc = this.hawkejs.compile({compiled: entry.callback, call: entry.path});
			}
