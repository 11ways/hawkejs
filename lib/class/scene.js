	...
	var Scene = Hawkejs.create(function Scene() {
		...
		this.openUrl = function openUrl(href, get, post, callback) {
			...
			function handleResponse(payload, textStatus, xhr) {
				...
				console.time('ClientRender')
				console.time('ClientExec');
				viewRender = hawkejs.render(payload, function clientRenderDone(err, html) {
					...
					console.timeEnd('ClientExec');

					blocks = viewRender.joinedBlocks;
					...
					for (blockName in blocks) {
						...
						element.dataset.origin = viewRender.mainTemplate;

						console.log('Default content was set to: ' + element.dataset['default-content']);

					}

					that.ajaxify();
					console.timeEnd('ClientRender')
