module.exports = function hawkejsSceneClass(HawkejsNS, Blast) {
	 ...
	 */
	var Scene = Function.inherits('Informer', 'Hawkejs', function Scene(parent) {
	...
	Scene.setMethod(function emitCreated(renderData) {
		...
		if (!document.body.getAttribute('data-origin')) {
			(function() {
				...
				if (!template) {
					return console.error('No lastTemplate rendered found?');
				}
	...
	Scene.setMethod(function fetch(href, options, callback) {
		...
		useUrl = Blast.Classes.RURL.parse(href);
		useUrl.addQuery(get);
		...
		useUrl.param('hajax', hajax);

		// Add time-on-page to post requests
		if (post) {
			useUrl.param('htop', ~~Blast.performanceNow());
		}
		...
		xhr.addEventListener('load', function transferComplete(e) {
			...
			if (typeof FileReader !== 'undefined') {
				...
				reader.onloadend = function onReaderLoadend() {
					...
					if (response.size && response.type.indexOf('json') > -1) {
		...
		if (post && post.constructor) {
			if (post.constructor.name == 'FormData') {
				is_form_data = true;
			} else if (typeof FormData != 'undefined' && post instanceof FormData) {
				is_form_data = true;
			}
		}

		if (post != null && !is_form_data) {
			xhr.setRequestHeader('content-type', 'application/json');
		}

		// Initialize the request
		Blast.setImmediate(function initializeFetch() {
	...
	Scene.setMethod(function openUrl(href, options, callback) {
		...
		if (options.diversion) {
			useUrl.param('h_diversion', options.diversion);
		}

		if (this.interceptOpenUrl) {
			pledge = this.interceptOpenUrl(useUrl, options);

			if (pledge) {
				pledge.then(function resolved() {
					if (callback) {
						callback();
					}
				});

				pledge.catch(function rejected(err) {
					doRequest(err);
				});
		...
		function doRequest(pledge_error) {
			...
			that.emit({type: 'ajax', state: 'begin'}, xhr, url, options, function doAjax() {
	...
	Scene.setProperty(function online_status_duration() {
		if (!this._online_change) {
			return ~~Blast.performanceNow();
		}

		return Date.now() - this._online_change;
	});
