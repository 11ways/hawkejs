module.exports = function hawkejsSceneClass(Hawkejs, Blast) {
	...
	Scene.setMethod(function attachHistoryListener() {
		...
		window.addEventListener('popstate', function onPopstate(e) {
			...
			if (e.state) {
				...
				console.log('Old content', content, e)
	...
	Scene.setMethod(function openUrl(href, get, post, callback) {
		...
		// Add a "hajax" GET parameter so browser's History API won't
		// show JSON data when doing things like "duplicate tab"
		Blast.Bound.URL.addQuery(useUrl, 'hajax', 1);
		...
		xhr.onreadystatechange = function onAjaxStateChange() {
			...
			reader.onloadend = function onReaderLoadend() {
				...
				that.serverResponse(url, result, xhr, callback);
			};
	 ...
	 */
	Scene.setMethod(function serverResponse(url, payload, xhr, callback) {
	...
	Scene.setMethod(function ajaxify() {
		...
		if (!this.enableClientRender) {
			console.log('Client side render disabled, not ajaxifying');
			return;
		}
