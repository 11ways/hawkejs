module.exports = function hawkejsSceneClass(Hawkejs, Blast) {
	...
	Scene.setMethod(function registerRender(options) {
		...
		// Merge live bindings
		Blast.Bound.Object.merge(this.live_bindings, options.live_bindings);
	...
	Scene.setMethod(function updateData(id, data) {
		...
		for (i = 0; i < arr.length; i++) {
			...
			if (!(entry = arr[i]) || !(element = document.getElementById(entry.element_id))) {
				arr[i] = undefined;
				continue;
			}
			...
			hawkejs.render(entry.fnc, variables, function gotResult(err, html) {
				element.innerHTML = html;
			});
		}
