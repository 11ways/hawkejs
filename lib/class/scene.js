	...
	Scene.setMethod(function serverResponse(url, options, payload, xhr, callback) {
		...
		if (options.history !== false) {
			...
			if (that.states.length) {
				payload.previous_state = that.states[that.states.length - 1];
			}

			// Store the state in the scene,
			// later it'll be pushed as a new state to the browser
			that.states.push(stateContent);
			payload.current_state = stateContent;
	...
	Scene.setMethod(function _insertDialog(filter, el, variables, viewRender) {
		...
		wrapper.addEventListener('click', function onClick(e) {
			...
			if (e.target == wrapper) {
				...
				if (viewRender && viewRender.previous_state) {
					state = viewRender.previous_state;
