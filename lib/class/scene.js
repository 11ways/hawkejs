module.exports = function hawkejsSceneClass(HawkejsNS, Blast) {
	...
	Scene.setMethod(function openUrl(href, options, callback) {
		...
		if (this.interceptOpenUrl) {
			...
			console.log('Intercepting open url, got pledge', pledge);
			...
			if (pledge) {
				...
				pledge.catch(function rejected(err) {
					console.log('Rejected:', err, 'doing request normally')
					doRequest(err);
				});
		...
		function doRequest(pledge_error) {
			...
			if (pledge_error) {
				console.error('Doing server request because of error', pledge_error);
			}
		...
		while (this.newLinks.length) {
			...
			if (href) {
				...
				link.addEventListener('click', function onLinkClick(e) {
					...
					console.log('Break segment:', this.dataset.breakSegment)
					...
					console.log('Opening url', options);
