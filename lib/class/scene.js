module.exports = function hawkejsSceneClass(Hawkejs, Blast) {
	...
	var Scene = Hawkejs.create(function Scene() {
		 ...
		 */
		this.getBlockElement = function getBlockElement(name, origin) {
		...
		this.registerRender = function registerRender(options) {
			...
			// Get scripts
			this.handleScripts(options);

			// Get styles
			this.handleStyles(options);

			// General handling of links, titles, ...
			this.generalPostRender(options);
		 ...
		 */
		this.emitCreated = function emitCreated(renderData) {
		 ...
		 */
		this.serverResponse = function serverResponse(url, payload, xhr) {
			...
			viewRender = hawkejs.render(payload, function clientRenderDone(err) {
				...
				that.handleScripts(viewRender);
				this.handleStyles(viewRender);
				...
				this.generalPostRender(viewRender);
			});
		};
		 ...
		 */
		this.generalPostRender = function generalPostRender(viewRender) {
		 ...
		 * @param    {String}   title
		 */
		this.setPageTitle = function setPageTitle(title) {
		 ...
		 */
		this.handleScripts = function handleScripts(viewRender) {
		 ...
		 * @param    {ViewRender}   viewRender
		 */
		this.handleStyles = function handleStyles(viewRender) {
			...
			styles.forEach(function eachStyle(parameters, index) {
				that.enableStyle.apply(that, parameters);
			});
		};
		 ...
		 */
		this.enableStyle = function enableStyle(instructions, options) {
			...
			if (Array.isArray(instructions)) {
				for (i = 0; i < instructions.length; i++) {
					this.enableStyle(instructions[i], options);
				}
				return;
			}
		 ...
		 * @param    {Object}   options
		 */
		this.disableStyle = function disableStyle(instructions) {
		 ...
		 */
		this.getScript = function getScript(path, force, callback) {
		 ...
		 * @param    {Object}   options
		 */
		this.queueScript = function queueScript(instructions, options) {
