module.exports = function(Hawkejs) {
	...
	ViewRender.prototype.checkFinish = function checkFinish() {
		...
		console.log('Executing callbacks: ' + this.callbacks.length)

		while (this.callbacks.length) {
			args = this.callbacks.pop();
...
console.log(typeof blockName)
		...
		if (this.running) {
			this.callbacks.push([blockName, callback]);
			return;
		}
		...
		console.log('Finishing ' + blockName + ' - ' + !!callback);
		...
		console.log(blockName + ' - Executing ' + tasks.length + ' tasks')
tasks[tasks.length] = function(next){
	console.log('BLAAAA')
	next();
}
