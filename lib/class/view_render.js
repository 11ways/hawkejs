module.exports = function(Hawkejs, Blast) {
	 ...
	 * @param    {Object}   options
	 */
	ViewRender.setMethod(function buildElement(tagName, options) {

		var tag = new Hawkejs.ElementBuilder(tagName, options);

		// Set the viewrender on the new builder
		tag.viewRender = this;
		...
		placeholder.setGetContent(function(done) {
			...
			setTimeout(function cheating() {
				...
				for (i = 0; i < that.headTags.length; i++) {
					...
					// Create the element
					element = that.buildElement(tag.name);
	...
	ViewRender.setMethod(function add_link(href, options) {
		...
		var content,
		    source,
		    anchor = this.buildElement('a', options),
		    name;
