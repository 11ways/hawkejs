module.exports = function(Hawkejs, Blast) {
	...
	/**
	 * Add client-side javascript from within the template
	 *
	 * @author   Jelle De Loecker   <jelle@develry.be>
	 * @since    1.0.0
	 * @version  1.0.0
	 ...
	 * @param    {Function}   fnc
	 */
	ViewRender.setMethod(function onCreate(fnc) {
	...
	ViewRender.setMethod(function foundation(options) {
		...
		placeholder.setGetContent(function(done) {
			...
			setTimeout(function cheating() {
				...
				dry = Utils.JSON.dry([settings, that]);
				...
				html += '<style type="text/css">.js-he-bottom {position:fixed;left:-100vw;}</style>\n';
	...
	ViewRender.setMethod(function bottom() {
		...
		placeholder.setGetContent(function(done) {
			...
			html = '<x-hawkejs class="js-he-bottom"></x-hawkejs>';
	...
	ViewRender.setMethod(function live(path, callback) {
		...
		value = Blast.Bound.Object.path(this.variables, path_pieces);
		...
		if (path_pieces.length > 1) {
			...
			// Get the parent
			parent = Blast.Bound.Object.path(this.variables, parent_pieces);
		} else {
		...
		this.print('<he-bound id="' + element_id + '" data-id="' + (id||'') + '" data-path="' + path + '">');
