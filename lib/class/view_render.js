module.exports = function(Hawkejs, Blast) {
	 ...
	 */
	ViewRender.setMethod(function implement(elementNames, variables) {
		...
		options = {
			// The content of a template to implement should never
			// be used as the main template
			changeMain: false,
		...
		this.execute(elementNames, variables || that.variables, options);
		...
		placeholder.setGetContent(function getPlaceholderContent(next) {
			...
			for (i = 0; i < elementNames.length; i++) {
				blockName = elementNames[i] + '__main__';

				if (options.blockNameModifier != null) {
					blockName += options.blockNameModifier;
				}

				if (that.blocks[blockName] != null) {
					break;
				}
			}

			that.finish(blockName, function getBlockHtml(err, result) {
				next(err, result);
			});
		});
	...
	ViewRender.setMethod(function print_element(elementNames, variables) {
		...
		subRender.execute(elementNames, variables || that.variables, true);
