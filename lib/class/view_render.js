 ...
 * @version  1.3.0
 */
ViewRender.setProperty(function active_block() {
...
ViewRender.setMethod(function createBlock(blockName, options) {
	...
	if (result == null) {
		...
		sub_options.startCall = null;
		sub_options.ancestor = result;

		blockName = blockName + '__expanded__';
		let new_block = this.createBlock(blockName, sub_options);
...
ViewRender.setMethod(function beginRender(names, variables, callback) {
	...
	Fn.parallel(function emitHawkejsBegin(next) {
		...
		that.emit('cloningVariables', that.variables, that.exposeToScene);
		...
		that.emit('clonedVariables', that.variables, that.exposeToScene);
	...
	placeholder.setGetContent(function getFoundationContent(done) {
		...
		that.waitForOtherPlaceholders(placeholder, function otherPlaceholdersDone(err) {
			...
			Fn.parallel(function getTitle(next) {
				...
				that.emit('preparingHeadTags', that.headTags);
				...
				that.prepareContent(that.headTags, function done(err) {
					...
					that.emit('preparedHeadTags', metas);

					next();
				});
			}, function getHtml(next) {

				var packed_variables,
				    url;
				...
				packed_variables = [that.variables, settings, that];

				that.emit('dryingVariables', packed_variables);

				// Get the serialized settings & view variables
				if (that.hawkejs._debug) {
					dry = Utils.JSON.dry(packed_variables, null, '\t');
				} else {
					dry = Utils.JSON.dry(packed_variables);
				}

				that.emit('driedVariables', dry);
				...
				html += '\t\t<script>window._initHawkejs = ' + dry.replace(/\<\/script/g, '</s" + "cript') + ';';
				...
				for (i = 0; i < that.styles.length; i++) {
					...
					for (j = 0; j < that.styles[i][0].length; j++) {
						...
						url = Blast.Classes.RURL.parse(style, that.hawkejs.root + that.hawkejs.stylePath);

						if (!url.hostname || url.usedBaseProperty('hostname')) {
							if (that.expose('version_info') || that.internal('version_info')) {
								url.param('v', that.expose('version_info') || that.internal('version_info'));
							}
 ...
 */
ViewRender.setMethod(function getBlockByName(block) {
...
ViewRender.setMethod(function print(content, block) {
	...
	return this.getBlockByName(block).push(content);
});
...
/**
 * Render a partial in the current ViewRender scope
 * and print it out.
 *
 * @author   Jelle De Loecker   <jelle@develry.be>
 * @since    1.0.0
 * @version  1.2.3
 *
 * @param    {Array|String}  partial_names    The element to render
 * @param    {Object}        options
 * @param    {Object}        variables
 */
ViewRender.setMethod(function implement(partial_names, options, variables) {
	...
	templates = new Hawkejs.Templates(this, partial_names);
	...
	if (!options.newscope) {
		...
		this.emit('subRender', renderer, partial_names);
	...
	hinder = Fn.hinder(false, function executeTemplates(done) {
		...
		// Start executing the wanted partial
	...
	placeholder.setGetContent(function getPlaceholderContent(next) {
		...
		hinder.push(function finishing() {
			...
			if (templates.active == null) {
				return next(new Error('No template was found for ' + manner + ' placeholder: "' + partial_names + '"'));
			}

			partial_name = templates.active.sourceName;

			if (phOptions.changeMain) {
				blockName = renderer.mainBlock;
				baseName = renderer.mainBlockBaseName;
			} else {
				baseName = blockName = partial_name + '__main__';
			...
			if (is_sub_render) {
				that.emit('finishingSubRender', renderer, partial_names);
			}
			...
			renderer.finish(blockName, function getBlockHtml(err, result, found, block) {
				...
				if (is_sub_render) {
					that.emit('finishedSubRender', renderer, partial_names);
				}
 ...
 * @param    {String}   name
 */
ViewRender.setMethod(function create_element(name, add_identifier) {
 ...
 * @param    {String}   name
 */
ViewRender.setMethod(function _print_element(name, options) {

	var element = this.create_element(name, false),
	    block = this.blocks[this.currentBlock];

	if (options) {
		element[options_symbol] = options;
	}

	block.pushElement(element);

	if (Hawkejs.Hawkejs.VOID_ELEMENTS[name.toUpperCase()]) {
		this._close_element(name);
	}
});
 ...
 * @param    {String}   name
 */
ViewRender.setMethod(function _close_element(name) {

	var options,
	    block = this.blocks[this.currentBlock],
	    el = block.elements.pop();

	options = el[options_symbol];
	...
	if (options) {
		...
		if (options.directives) {
			...
			for (i = 0; i < options.directives.length; i++) {
				...
				context = Blast.Bound.Object.path(this.helpers, val.context);
				...
				if (!val.method) {
					throw new Error('@TODO: implement non-method directive');
				}

				if (!context[val.method]) {
					throw new Error('Unable to find "' + val.method + '" directive');
				}

				context[val.method](el, options);
			}
		}
	}
...
/**
 * Render a partial in a new ViewRender scope and print it out.
 ...
 * @version  1.2.3
 *
 * @param    {Array|String}  partial_names    The partial to render
 * @param    {Object}        variables
 * @param    {Object}        options
 */
ViewRender.setMethod(['print_partial', 'print_element'], function print_partial(partial_names, variables, options) {
	...
	return this.implement(partial_names, options, variables);
});
...
ViewRender.setMethod(function waitForOtherPlaceholders(exclude_placeholder, callback) {
	...
	this.emit('waitingForOtherPlaceholders');
	...
	function finish(err) {
		that.emit('waitedForOtherPlaceholders', err);
		callback(err);
	}
});
 ...
 * @version  1.2.9
 */
ViewRender.setMethod(function startExpression(name, options, vars, fnc) {

	var instance = new Blast.Classes.Hawkejs.Expression[name](this);
 ...
 * @version  1.3.3
 */
ViewRender.setMethod(function parseExpression(tokens, vars) {
	return Blast.Classes.Hawkejs.Expression.Expression.parseExpression(this, tokens, vars);
});
 ...
 * @version  1.3.2
 *
 * @param    {ViewRender}   view_render
 */
HelperCollection = Fn.inherits('Informer', 'Hawkejs', function HelperCollection(view_render) {
	this.view_render = view_render;
});

Hawkejs.ViewRender = ViewRender;
Hawkejs.HelperCollection = HelperCollection;