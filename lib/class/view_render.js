module.exports = function(Hawkejs, Blast) {
	...
	if (typeof WeakMap == 'function') {
		if (typeof Blast.Globals.Proxy != 'undefined') {
	...
	ViewRender.setMethod(function createBlock(blockName, options) {
		...
		if (this.blocks[blockName] == null) {
			this.blocks[blockName] = new Hawkejs.BlockBuffer(this, blockName, options);
		}
	...
	ViewRender.setMethod(function start(blockName, options) {
		...
		if (!block.variables) {
			block.variables = this.protectVariables(this.variables, true);
		}
	...
	ViewRender.setMethod(function beginRender(names, variables, callback) {
		...
		// Clone the variables
		this.variables = this.protectVariables(this.variables);
	...
	/**
	 * Protect variables by creating a proxy,
	 * or cloning the object if no proxy is available
	 *
	 * @author   Jelle De Loecker   <jelle@develry.be>
	 * @since    1.0.0
	 * @version  1.0.0
	 *
	 * @param    {Object}   variables   The variables to protect
	 * @param    {Boolean}  shallow     Wether to protect them shallowly
	 */
	ViewRender.setMethod(function protectVariables(variables, shallow) {
		...
		// When there is no proxy object, just create a shallow clone
		if (!Sproxy && shallow) {
			return Utils.Object.assign({}, variables);
		}

		// Fallback to an expensive cloning operation
		if (!Sproxy) {
			return Blast.Collection.JSON.undry(Blast.Collection.JSON.dry(variables));
		}

		// Create the variable weakmap
		vwm = new WeakMap();

		// Create a completely empty sandbox
		box = Object.create(null);

		handler = {
			get: function getProperty(target, name) {

				var result;

				if (box[name] != null || name in box) {
					result = box[name];
				} else {
					result = target[name];

					// Proxy other objects too
					if (result && typeof result == 'object') {

						if (!Object.isFrozen(result)) {
							result = that.protectVariables(result);
						}
			...
			set: function setProperty(target, name, value) {
				box[name] = value;
			}
		};

		// Create a proxy of an augmented variables object
		proxy = new Sproxy(Object.create(variables), handler);
		...
		placeholder.setGetContent(function(done) {
			...
			setTimeout(function cheating() {
				...
				for (i = 0; i < that.headTags.length; i++) {
					...
					// Create the element
					element = Hawkejs.ElementBuilder.create(tag.name);
	...
	ViewRender.setMethod(function add_link(href, options) {
		...
		if (options.history === false) {
			anchor.attr('data-history', false);
		}
					...
					if (options.wrap) {
						...
						if (block && block.attributes) {
							for (key in block.attributes) {
								element.attr(key, block.attributes[key]);
							}
						}
