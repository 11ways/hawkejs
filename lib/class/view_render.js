 ...
 * @param    {String}   name
 */
ViewRender.setMethod(function print_element_placeholder(name, options) {

	var element = this.create_element(name),
	    placeholder = new Hawkejs.ElementPlaceholder(this),
	    block = this.blocks[this.currentBlock];

	placeholder.element = element;

	block.pushElement(placeholder);
});
 ...
 * @param    {String}   name
 */
ViewRender.setMethod(function close_element_placeholder(name) {

	var block = this.blocks[this.currentBlock],
	    last = block.elements[block.elements.length - 1];

	//console.log('closing', name, '?', last);
