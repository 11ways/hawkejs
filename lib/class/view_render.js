module.exports = function(Hawkejs, Blast) {
	...
	ViewRender.prototype.beginRender = function beginRender(templateName, variables, callback) {
		...
		log(0, 'Started rendering', templateName);
		...
		if (this.clientRender && Blast.isNode) {
			...
			log(0, 'Deferring render of', templateName, 'back to client');
		...
		log(1, 'Executing', templateName);

		// Execute the template code, and schedule the callback
		this.execute(templateName, variables, true, function afterExec() {

			log(1, 'Executed main template code of', templateName, ', requesting finish');

			that.finish(function afterFinish(err, html) {
				log(1, 'Render that began with', templateName, 'has finished. HTML length is', html.length);
	...
	ViewRender.prototype.execute = function execute(name, variables, changeMain, callback) {
		...
		this.hawkejs.getCompiled(name, function gotCompiledTemplate(err, fnc) {
			...
			if (err) {
				log(4, 'Failed to get compiled template', name, 'with error', err);
				return that.hawkejs.handleError(that, name, 'pre', err);
			}
	...
	ViewRender.prototype.finish = function finish(blockName, callback) {
		...
		if (this.running) {
			...
			log(4, 'I/O still busy, rescheduling finish for', blockName);

			this.after('ioDone', function afterIO() {
				log(4, 'I/O is done, resuming finish for', blockName);
		...
		if (!blockName) {
			...
			log(4, 'Finishing main block', blockName);
		} else {
			log(4, 'Finishing regular block', blockName);
		}
	 ...
	 * @param    {String}   layout
	 */
	ViewRender.prototype.expands = function expands(layout) {
	 ...
	 * @param    {String}   title
	 */
	ViewRender.prototype.set_title = function set_title(title) {
	 ...
	 */
	ViewRender.prototype.script = function script(instructions, options) {
	 ...
	 * @param    {Object}         options
	 */
	ViewRender.prototype.style = function style(instructions, options) {
	 ...
	 * @param    {Mixed}    value
	 */
	ViewRender.prototype.set = function set(name, value) {
	 ...
	 * @param    {Mixed}    value
	 */
	ViewRender.prototype.internal = function internal(name, value) {
	 ...
	 * @param    {Mixed}    value
	 */
	ViewRender.prototype.expose = function expose(name, value) {
