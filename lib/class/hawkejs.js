 ...
 * @return   {Node|Array}
 */
Hawkejs.setStatic(function objectToElement(input, parent, extras) {
	...
	if (input instanceof Blast.Classes.Hawkejs.Node) {
		return input;
	}
	...
	if (input.type == 'tag') {
		...
		node = this.createText(input.data);
		Object.assign(node, extras);
	}
...
Hawkejs.setMethod(function registerRender(options) {
	...
	// Create a new scene
	if (!this.scene) {
		this.scene = new Blast.Classes.Hawkejs.Scene(this, options);
	} else {
 ...
 * @return   {String}
 */
Hawkejs.setMethod(function parseDirectives(source) {

	var instance = new Blast.Classes.Hawkejs.Parser.Directives(source),
 ...
 * @return   {String}
 */
Hawkejs.setMethod(function parseExpressions(source) {
	...
	lines = Utils.String.dissect(source, '{%', '%}');
	result = [];
	...
	for (i = 0; i < lines.length; i++) {
		...
		// Tokenize the line
		tokens = new Blast.Classes.Hawkejs.Parser.Expressions(line.content);

		added = false;

		for (key in Blast.Classes.Hawkejs.Expression) {

			if (typeof Blast.Classes.Hawkejs.Expression[key] != 'function') {
				continue;
			}
			...
			temp = Blast.Classes.Hawkejs.Expression[key].parse(options);
		...
		if (!added) {
			result.push('<% __render.parseExpression(' + JSON.stringify(tokens.getExpression()) + ', vars) %>');
		}
	}
...
Hawkejs.setMethod(function interpretTemplate(source, name) {
	...
	source = this.parseDirectives(source);
	source = this.parseExpressions(source);
	...
	for (i = 0; i < lines.length; i++) {
		...
		if (line.type == 'inside') {
			...
			if (!slurp || (slurp && line.content.trim())) {
	...
	if (options.evaluate !== false) {
		try {
			...
			if (typeof console !== 'undefined') {
				console.log('Found error when compiling view ' + json_name + ':')
				console.log(err);
				console.log(code);
			}
 ...
 * @version  1.2.8
 */
Hawkejs.setStatic(function createText(data) {
