...
HawkejsClass = Hawkejs = Nuclei.extend(function Hawkejs() {
	...
	this.compile = function compile(name, source) {
		...
		console.log(util.inspect(esprima.parse(code), {depth: null}));

		console.log('\n\nStart walking: »»»»»»»»»»»»»»»»»»»»»»»»»»»» \n')

		var ast = astronaut(code).walk(function(node) {
			...
			if (node.isIdentifier()) {
				...
				if (parent.isExpressionStatement()) {
					...
					if (parent.isMemberExpression() && parent.data.object.data.name == name) {
						main = true;
					}
				}

				if (main) {
					console.log(node.deparse())
					node.wrap('VARS.<%= node %>');
				}
				...
				console.log(util.inspect(node.parent, {depth: null}))
				//if (node.parent) console.log(node.parent.parent)
				console.log('<<<<\n')
			};

			return

			if (!node.parent || node.parent.isExpressionStatement()) {
				if (node.isIdentifier()) {
					console.log(node.deparse())
				}
			}
