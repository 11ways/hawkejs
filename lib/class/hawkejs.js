...
HawkejsClass = Hawkejs = Nuclei.extend(function Hawkejs() {
			...
			if (line.type == 'inside') {
				...
				if (this.commands[cmd]) {
					...
					for (j = 0; j < temp.length; j++) {
						...
						if (j > 0) {
							code += tab + 'this.setErr(' + strName + ',' + (line.lineStart+j) + ');\n';
						}

						code += tab + temp[j].trim() + ';\n';
					}
				}
	...
	this.handleError = function handleError(viewRender, templateName, lineNr, error) {
		...
		if (lineNr == 'pre') {
			...
			templateName = viewRender.lastTemplate;
	...
	this.getCompiled = function getCompiled(templateName, callback) {
		...
		if (this.templates[templateName]) {
			return setImmediate(function(){callback(null, that.templates[templateName])});
		}
