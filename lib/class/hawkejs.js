var Blast      = require('protoblast')(false), // Should be false when dev is done
    Utils      = Blast.Bound,
...
Hawkejs.setMethod(function compile(name, source) {
	...
	for (i = 0; i < lines.length; i++) {
		...
		if (line.type == 'inside') {
			...
			// Get the cmd keyword
			cmd = Blast.Bound.String.trimLeft(line.content).split(' ')[0];
			...
			if (this.commands[cmd]) {
				...
				for (j = 0; j < temp.length; j++) {
					...
					// inspecting later
					blockCode += tab + '/*lineNr:' + (line.lineStart+j) + '*/ ';
				...
				if (temp[0][0] == '}' || temp[temp.length-1][temp[temp.length-1].length-1] == '{') {
					...
					if (Blast.Bound.String.count(blockCode, '{') == Blast.Bound.String.count(blockCode, '}')) {
						blockCode = 'try { ' + blockCode + '} catch (err) {';
						blockCode += 'this.hawkejs.handleError(this, this.errName, this.errLine, err);'
						blockCode += '}';
					}
				}
...
Hawkejs.setMethod(function getSource(templateName, callback) {
	...
	bomb = Blast.Collection.Function.timebomb(30000, callback);
