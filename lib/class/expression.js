...
If.setStatic(function parseArguments(options) {
	...
	//result = parseExpression(options.tokens);
...
If.setMethod(function execute() {
	...
	var variable = parseNewArgs(this.options, this.vars),
	    vars,
	    i;
...
function parseNewArgs(args, vars) {
	...
	for (i = 0; i < args.length; i++) {
		...
		b = getValue(args[++i], vars);

		console.log('-- B:', args[i])
...
function getValue(token, vars) {
	...
	if (token.variable) {
		let res =  getPart(token.variable, vars);
		console.log(token.variable, '===', res, vars)
		return res;
	}

	if (Array.isArray(token)) {
		return parseNewArgs(token, vars);
	}
}
