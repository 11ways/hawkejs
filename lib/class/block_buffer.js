module.exports = function(Hawkejs, Blast) {
	...
	// Get all the properties of the Array prototype,
	// even the non-enumerable ones
	ArrayProperties = Object.getOwnPropertyNames(Array.prototype);

	// Inherit the array methods
	for (i = 0; i < ArrayProperties.length; i++) {
		name = ArrayProperties[i];

		// Don't inherit the constructor
		if (name == 'constructor') {
			continue;
		}

		BlockBuffer.prototype[name] = Array.prototype[name];
	}
