module.exports = function hawkCustomElement(Hawkejs, Blast) {
	...
	Element.setStatic(function setAttribute(name, getter, setter, config) {
		...
		this.setProperty(name, function attrGetter() {
			...
			if (getter) {
				val = getter.call(this, map.prop_values[name]);
			} else {
				// If there is no custom getter, just get the string
				val = map.prop_values[name];
			}
			...
			// this will also set it on the weakmap
	...
	Element.setProperty(function assigned_data() {
		...
		if (Blast.isBrowser && this.dataset.hid && browser_store[this.dataset.hid]) {
			this._assigned_data = browser_store[this.dataset.hid];
			delete browser_store[this.dataset.hid];
		}
	...
	Element.setMethod(function toHawkejsString(viewRender) {
		...
		if (this.hawkejs_id) {
			...
			// On the client-side custom elements are turned into
			// strings before being added to the DOM. This is how
			// the assigned data will "survive" this transition
			if (Blast.isBrowser && this._assigned_data) {
				browser_store[this.hawkejs_id] = this._assigned_data;
			}
		}
