	...
	var BlockPlaceholder = Placeholder.extend(function BlockPlaceholder() {
		...
		this.init = function init(parent, blockName) {
			...
			this.parent('init', null, parent, function(next) {
				parent.finish(blockName, function(err, blockResult, exists) {

					var tasks = {};

					if (!exists && that.defaultContent) {
						tasks.defaultContentHtml = function joinDefaultContentBuffer(next) {
							that.parent.joinBuffer(that.defaultContent, next);
						};
					}

					async.parallel(tasks, function(err, taskResult) {

						var html;

						html = '<x-hawkejs data-type="block" data-name="' + blockName + '" data-origin="' + that.origin + '"';
