module.exports = function hawkElementBuilder(Hawkejs, Blast) {
	...
	var skipTags = ['id', 'class', 'for', 'name'];
	...
	ElementBuilder.prototype.getOpenTagString = function getOpenTagString() {
		...
		// If tabindex has been specified in the options,
		// and not yet set as an attribute, set it now
		if (this.tabindex != null && !this.getSerialized('tabindex')) {

			if (this.tabindex === false) {
				temp = -1;
			} else if (this.tabindex === true) {
				temp = 0;
			} else {
				temp = this.tabindex;
			}

			this.setAttribute('tabindex', temp);
		}
