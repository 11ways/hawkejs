module.exports = function hawkejsServer(HawkejsNS, Blast) {
	...
	Hawkejs.setMethod(function getTemplatePath(templateName, callback) {
		...
		if (!this._debug && this.templatePathCache[templateName]) {
	 ...
	 */
	HawkejsNS.ViewRender.setMethod(function prepare(req, res) {
		...
		if (history_url) {
			...
			Blast.Bound.URL.addQuery(url, 'htop', null);
			Blast.Bound.URL.addQuery(url, 'h_diversion', null);
	...
	Hawkejs.setMethod(function getFirstAvailableInternalSource(names, callback) {
		...
		Utils.Function.while(function test() {
			...
			that.getSource(template_name, function gotTemplateSource(err, source) {
			...
			if (!found_source) {
				return callback(new Error('Found no available sources'));
			}
