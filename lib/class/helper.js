/**
 * The Helper Class
 *
 * @author   Jelle De Loecker   <jelle@develry.be>
 * @since    1.0.0
 * @version  1.2.3
 *
 * @param    {ViewRender}   view
 */
var Helper = Blast.Collection.Function.inherits('Informer', 'Hawkejs.Helper', function Helper(view) {

	if (!view && typeof hawkejs != 'undefined') {
		view = new Hawkejs.ViewRender(hawkejs);
	}

	// Set the view render instance
	this.view = view;
});

/**
 * Register the helper
 *
 * @author   Jelle De Loecker <jelle@develry.be>
 * @since    1.2.3
 * @version  1.3.2
 */
Helper.constitute(function registerHelper() {

	if (this.name == 'Helper') {
		return;
	}

	let that = this,
	    name = Blast.Bound.String.beforeLast(this.name, 'Helper') || this.name;

	if (Hawkejs.Hawkejs.helpers[name]) {
		return;
	}

	let view_symbol = Symbol(name);

	Hawkejs.HelperCollection.setProperty(name, function getHelper() {

		if (!this[view_symbol]) {
			this[view_symbol] = new that(this.view_render);
		}

		return this[view_symbol];
	});

	Hawkejs.Hawkejs.helpers[name] = this;
});

/**
 * Print out something to the view
 *
 * @author   Jelle De Loecker   <jelle@develry.be>
 * @since    1.0.0
 * @version  1.0.0
 */
Helper.setMethod(function print(data) {
	this.view.print(data);
});

/**
 * Generate an RURL instance
 *
 * @author   Jelle De Loecker   <jelle@develry.be>
 * @since    1.0.0
 * @version  1.2.4
 */
Helper.setMethod(function parseURL(href, origin) {

	var result;

	// If it already is some kind of URL object, parse it again
	if (href && typeof href == 'object') {
		return Blast.Classes.RURL.parse(href, origin);
	}

	// Get the internal urlOrigin setting, if it's available
	if (!origin && this.view) {
		origin = this.view.internal('urlOrigin');
	}

	result = Blast.Classes.RURL.parse(href, origin);

	return result;
});

/**
 * Create and return a placeholder
 *
 * @author   Jelle De Loecker   <jelle@develry.be>
 * @since    1.0.0
 * @version  1.2.2
 *
 * @param    {Object}     options
 * @param    {Function}   fnc
 */
Helper.setMethod(function placeholder(options, fnc) {
	return this.view.placeholder(options, fnc);
});

/**
 * Create a new element
 *
 * @author   Jelle De Loecker   <jelle@develry.be>
 * @since    1.2.9
 * @version  1.2.9
 *
 * @param    {String}   name
 */
Helper.setMethod(function createElement(name) {
	var element = this.view.create_element(name);
	return element;
});