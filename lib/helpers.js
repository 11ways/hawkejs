module.exports = function helpers (hawkejs) {
	...
	hawkejs.drones.flowExample = function(done, $result) {
		setTimeout(function() {
			...
			var $s = hawkejs.utils.select($result, 'hawkejs[data-type="droneExample"]');
			$s.replaceWith('DONE');
			done();
		}, 100);
	};
	 ...
	 */
	helpers.encode = hawkejs.utils.encode;
	
	/**
	 * Expose the stringify function
	 */
	helpers.stringify = function() {
		return hawkejs.stringify.apply(null, arguments);
	};
	...
	helpers.parse = function() {
		return hawkejs.parse.apply(null, arguments);
	}

	/**
	 * Expose the clone function
	 */
	helpers.clone = hawkejs.utils.cloneobject;
	...
	helpers.echo = function echo(message, options) {
		...
		if (options && options.escape) {
			message = hawkejs.utils.encode(message);
		}
	...
	helpers.render_end = function() {
		...
		for (var blockname in ta) {
			...
			if (!tab.finished) {
				...
				if (tab.parent == '_') {
					pblock = this.scope.buf;
				} else {
					if (this.request.blocks[tab.parent]) {
						pblock = this.request.blocks[tab.parent].buf;
					}
				}
	...
	helpers.start = function start(blockname, options) {
		...
		// Add a single empty string to the buffer to prevent any 'undefined' messages
		this.scope.buf.push('');
	 ...
	 * @param    {object}   options
	 */
	helpers.print_element = function print_element(elementname, options, callback) {
		
		var payload = this,
		    newVars = {},
		    savebuf = this.scope.buf,
		    result,
		    html,
		    r;
		...
		if (options.fullrender) {
			hawkejs.utils.inject(newVars, payload.__variables, options.variables);
			hawkejs.render(elementname, newVars, function(element, a, result) {
				callback(element, result);
			});
		} else {

			// Create a new buffer without touching the old one
			this.scope.buf = [];

			// Shallowly clone the payload object
			payload = hawkejs.utils.inject({}, this);

			// Have we given specific variables?
			if (options.variables !== undefined) hawkejs.utils.inject(payload, options.variables);

			if (options.newscope) {
				// Create a new request object
				payload.request = {};
				
				// Inject the request objects
				hawkejs.utils.inject(payload.request, this.request);
			...
			result = hawkejs._EjsRender(elementname, payload,
	...
	helpers.script = function script(scriptpath, options) {
		...
		// Go over every path in the array and add them
	...
	helpers._add_match_options = function _add_match_options (elementid, instructions) {
		...
		// If no valid href is given, do nothing
		if (!href) return;
			...
			async.cleaner = function asyncCleaner() {
				...
				if (async.done && async.called === async.counter) {
					for (asyncId in async.results) {
						...
						$s = hawkejs.utils.select(async.$result, 'hawkejs[data-async-id="'+asyncId+'"]');
