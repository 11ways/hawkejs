module.exports = function helpers (hawkejs) {
	...
	helpers.print_element = function print_element(elementname, options, callback) {
		...
		if (options.fullrender) {
			hawkejs.µ.inject(newVars, payload.__variables, options.variables);
			...
			payload = hawkejs.µ.inject({}, this);

			// Have we given specific variables?
			if (options.variables !== undefined) hawkejs.µ.inject(payload, options.variables);
			...
			if (options.newscope) {
				...
				hawkejs.µ.inject(payload.request, this.request);
