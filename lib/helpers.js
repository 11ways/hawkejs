module.exports = function helpers (hawkejs) {
	...
	/**
	 * Bind all the functions to the proper "this" context
	 * 
	 * @author   Jelle De Loecker   <jelle@kipdola.be>
	 * @since    2013.06.03
	 * @version  2013.06.03
	 */
	var bindFunctions = function bindFunctions (object, context) {
		...
		for (var i in object) {
			...
			if (typeof object[i] == 'function') {
				...
				if (Object.prototype.toString.call(object[i]) == "[object Object]"
						&& object.hasOwnProperty(object[i])) {
					bindFunctions(object[i], context);
				}
			}
		}
	}
	...
	helpers.render_start = function (buf, stack) {
		...
		for (var i in this) {
			if (i == 'scope' || i == 'request' || i == 'settings'
			    || i == 'hawkejs' || i == '_locals' || i == 'prive') {
				// Skip these objects
			} else {
				bindFunctions(this[i], this);
			}
		}
